...
| Photosensitivity boot-warning modal | **MVP** | `ModalScaffold` child of MainMenu, `PhotosensitivityWarningContent` child | push `Context.MODAL` on `show_modal()`; pop on Acknowledge | "Continue" (per Settings CR-18 default) | `dialog` (live `assertive` one-shot) | "Acknowledge" button only — **NOT** dismissible by `ui_cancel` (CR-8) |
...
| Quit-confirm modal | **MVP** | `ModalScaffold` child of MainMenu (and PauseMenu at VS) | push `Context.MODAL`; pop on Cancel/Confirm | "Return to File" (Cancel) — safe-default | `dialog` (live `assertive` one-shot) | "Return to File" / `ui_cancel` / "Close File" (latter triggers `quit()`) |
...
5. **Boot-warning poll.** Synchronous read of `SettingsService._boot_warning_pending` (CR-8). **Branch A (`true`):** `ModalScaffold.show_modal(PhotosensitivityWarningContent)`; `Context.MODAL` pushed; modal awaits Acknowledge button; on Acknowledge: `SettingsService.dismiss_warning()` writes dismissed flag, `ModalScaffold.hide_modal()`, `Context.MODAL` popped, button container `set_process_input(true)`, focus to "Continue"/"Begin Operation". **Branch B (`false`):** button container immediately enabled; focus set per CR-5 default.
...
| MainMenu → ModalScaffold (Photosensitivity) | **Blocked.** No effect. Only "Continue"/"Acknowledge" button dismisses (CR-8). |
...
| `menu.photo_warning.continue` | Acknowledge (11 chars) — Menu-owned button label inside the scaffold |
...
| Photo Acknowledge button | `button` | `tr("menu.photo_warning.continue")` → "Acknowledge" | (none) | `off` | MVP |
...
| **Settings & Accessibility** ✅ | Menu → Settings | Reads `_boot_warning_pending: bool` synchronously in `_ready()` (CR-8); calls `open_panel()` from Settings entry-point (CR-7); calls `dismiss_warning()` on Acknowledge | Settings CR-18 + OQ-SA-3 | **CLOSES Settings BLOCKING coord OQ-SA-3** with this GDD's CR-8 + C.4 ModalScaffold spec |
...
- **If gamepad disconnect occurs during the non-dismissible photosensitivity boot-warning modal**: Keyboard "Enter" / `ui_accept` still works on Acknowledge (FOCUS_ALL responds to KB and gamepad). The `joy_connection_changed` handler queues the transient disconnect card — but the modal has `Context.MODAL` and button container has `set_process_input(false)`. The transient card must NOT push InputContext (same non-modal feedback pattern). Renders above modal at CanvasLayer 20+, auto-fades per F.4 timeline. Mouse cursor still works. Player can click Acknowledge with mouse or press Enter.
...
5. **Settings GDD coord — `dismiss_warning()` returns bool**: detect disk-full failure at boot-warning Acknowledge. Required by Cluster A case 5.
...
| **Settings & Accessibility** ✅ | Approved (pending OQ-SA-3 closure by this GDD) | Synchronous read of `SettingsService._boot_warning_pending: bool` in `MainMenu._ready()` (Settings CR-18). Calls `SettingsService.open_panel()` from Settings entry-point button (CR-7). Calls `SettingsService.dismiss_warning()` on Acknowledge button (modal scaffold). Subscribes to `Events.setting_changed` filtered for `category == "accessibility" && name == "reduced_motion_enabled"` per CR-23. |
...
| `accessibility.photosensitivity_warning_dismissed` | **Settings & Accessibility** | Menu CR-8 poll | Settings on Acknowledge button + `[Show Photosensitivity Notice]` button |
...
1. `design/ux/photosensitivity-boot-warning.md` — Day-1 MVP, **HARD-blocking HUD Core + Settings sprints**. Modal scaffold layout, Acknowledge / Go to Settings button placement, AccessKit announce timing.
...
- **AC-MENU-6.3 [Logic] [BLOCKING]** GIVEN photosensitivity modal is open, WHEN Acknowledge is activated, THEN in order: (a) `SettingsService.dismiss_warning()` is called and returns `true`; (b) `ModalScaffold.hide_modal()` is called; (c) `peek() == Context.MENU`; (d) focus moves to Continue / Begin Operation per CR-5; (e) Main Menu button container has `process_input = true`. All five within one frame. Verifies CR-8 Acknowledge path.
- **AC-MENU-6.4 [Logic] [BLOCKING]** GIVEN `dismiss_warning()` returns `false` (disk-full failure), WHEN Acknowledge is pressed, THEN modal remains open, `peek() == Context.MODAL`, button container retains `process_input = false`, and a second Acknowledge press re-attempts `dismiss_warning()`. Verifies §E Cluster A case 5.
- **AC-MENU-6.5 [UI] [ADVISORY]** GIVEN photosensitivity modal is open with screen reader active, WHEN the modal appears (assertive one-shot per F.7), THEN the AT announces the modal's content. Default focus = Acknowledge. Go to Settings button has `accessibility_description`. Manual walkthrough doc filed. Verifies CR-8, C.9.

### H.6-locale (NEW per QA-lead GAP-3)

- **AC-MENU-6.6 [Logic] [BLOCKING — before any non-EN locale ships]** GIVEN a localised body string for the photosensitivity warning, WHEN `Label.get_line_count()` is called after rendering with `autowrap_mode` set, THEN the result is ≤ 5. For strings ≤ 300 chars: rendered at 11 px. For 301–345 chars: rendered at 10 px. For ≥ 346 chars: locale is flagged as GDD amendment required before shipping (CI check fails the locale bundle). Verifies F.2.
