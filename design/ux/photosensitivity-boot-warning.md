...
|---|---|---|---|---|
| **Z1 — Title bar** | Top | 0–14% V (≈40 px) | Modal title `tr("SETTINGS_PHOTOSENSITIVITY_WARNING_TITLE")` ("Photosensitivity Notice") — left-aligned, Futura/DIN bold 24 px (line-height ≈ 30 px), BQA Blue on Parchment-tinted modal frame; thin 1 px Ink Black rule below | MVP |
...
| Component | Type | Content | Interactive | Pattern |
|---|---|---|---|---|
| `TitleLabel` | `Label` | `tr("SETTINGS_PHOTOSENSITIVITY_WARNING_TITLE")` ("Photosensitivity Notice"); Futura/DIN bold 24 px (line-height ≈ 30 px), BQA Blue on Parchment, left-aligned | No | `auto-translate-always` |
...
│ ░░       │  or check disk space.                                       │       ░░ │     (Ink Black 16 px italic)
│ ░░       │                                                              │       ░░ │
│ ░░       │ ──────────────────────────────────────────────────────────── │       ░░ │
│ ░░       │              ┌──────────────────┐  ┌──── ▶ ─────────┐       │       ░░ │
...
| Failure feedback (`FailureFeedbackLabel`, 16 px italic) | Ink Black `#1A1A1A` | Parchment `#E8DCC4` | ≥ 12:1 | Designed |
...
- Title: 24 px (meets the 24 px menu UI floor per `accessibility-requirements.md`. OQ #4 RESOLVED — scaled from 22 px. 2 px increase absorbed in existing Z1 margins; no knock-on to overall modal height.).
...
- Failure feedback: 16 px italic (meets SC 1.4.4 informational text floor. OQ #5 RESOLVED — scaled from 14 px. Italic styling preserved; this label is informational/action-directing, not decorative chrome).
...
|---|---|---|---|---|---|
| `SETTINGS_PHOTOSENSITIVITY_WARNING_TITLE` | "Photosensitivity Notice" | 23 | ~50 chars (Z1 title bar at 24 px Futura/DIN bold) | ≤ 32 | ✓ likely fits FR ("Avis de photosensibilité" ~24 chars) and DE ("Hinweis zur Photosensibilität" ~30 chars) |
...
| `SETTINGS_PHOTOSENSITIVITY_DISMISS_FAILED` | "Could not save your acknowledgment. Please try again or check disk space." | 73 | ~80 chars × 1 line @ 16 px italic in Z4 | ≤ 102 | ⚠ FR "Échec de l'enregistrement de votre acceptation. Veuillez réessayer ou vérifier l'espace disque." ~98 chars — fits but tight |
...
| Failure feedback width | `FailureFeedbackLabel` at 16 px italic must fit in 1 line within Z2 width (~~~70 chars at 16 px italic — narrower than the prior 14 px estimate). | If FR/DE pushes past 70 chars, allow 2-line wrap (modal grows another 22 px tall). Don't shrink the font. Per OQ #5 resolution 2026-04-29. |
...
- **AC-PSBW-3.2 [Logic] [BLOCKING]** GIVEN the modal mounted, WHEN `[Go to Settings]` is activated AND `dismiss_warning()` returns `true`, THEN: (a) dismissed-flag persisted same as AC-PSBW-3.1; (b) `hide_modal()` called; (c) `Context.MODAL` popped; (d) `SettingsService.open_panel(pre_navigate: "accessibility.damage_flash_enabled")` called; (e) `Context.SETTINGS` pushed by Settings (condition (e) is BLOCKED-pending OQ #10 resolution; can be tested only after Settings GDD locks the pre_navigate key-path format); (f) focus on `damage_flash_enabled` toggle in Settings panel.
...
| **3** | ~~Audio register for this modal — silence (current spec), system-default UI click, or Pillar 5 typewriter SFX (rejected by carve-out)?~~ **RESOLVED** — Silence confirmed. Review-mode `ui_cancel` is allowed to dismiss (consistent with the normative spec body — States §D, Interaction Map §E, AC-PSBW-5.3 — which all specify dismiss IS allowed in review-mode; the boot-mode AC-MENU-6.2 lock applies only to first-launch boot path, not the player-initiated review path). Per `/ux-review` 2026-04-29. | Section E3 | sound-designer + ux-designer + accessibility-specialist | Resolved. | — |
| **4** | ~~TitleLabel at 22 px is BELOW the menu UI floor of 24 px from `accessibility-requirements.md`. Bump to 24 px, OR classify as decorative per art-bible §7B?~~ **RESOLVED** — TitleLabel scaled from 22 px to 24 px to meet menu UI text floor per `accessibility-requirements.md`. Per `/ux-review` 2026-04-29. | Section G (Min text sizes) | accessibility-specialist + art-director | Resolved. | — |
| **5** | ~~FailureFeedbackLabel at 14 px italic is BELOW SC 1.4.4 floor. Classify as decorative (matches main-menu.md build stamp precedent), or raise to 16 px?~~ **RESOLVED** — FailureFeedbackLabel scaled from 14 px to 16 px. This label is informational/action-directing ("Please try again or check disk space") and not decorative chrome; main-menu.md build-stamp decorative precedent does not apply. Per `/ux-review` 2026-04-29. | Section G | accessibility-specialist | Resolved. | — |
...
| **7** | menu-system.md AC-MENU-6.6 specifies font scale-down (11 px → 10 px) for non-EN locales whose body exceeds 5-line cap. **This conflicts with `accessibility-requirements.md` minimum text size** (24 px menu UI / 18 px Mission Card body). | Section H (layout-critical) | **Owner: game-designer (menu-system.md amendment). Deadline: before VS sprint kickoff.** | **Recommended: amend AC-MENU-6.6** to keep 18 px body and instead allow modal height to grow per locale (default 280 px → up to ~340 px). Reject font scale-down. | **BLOCKING** — must resolve before any non-EN locale ships. Flag back to menu-system.md GDD for amendment. |
