...
- **2026-04-29 (Cutscenes & Mission Cards amendment — closes `cutscenes-and-mission-cards.md` CR-CMC-11 BLOCKING coord + `audio.md` L407 forward-dep)**: Two coordinated signal additions land atomically. Signal count grows **41 → 43**; domain count grows **12 → 13** (NEW Cutscenes domain).

  1. **NEW signal `cutscene_started(scene_id: StringName)`** added to Cutscenes domain. Fires from `CutscenesAndMissionCards` (CanvasLayer 10 lazy-instance, NOT autoload per cutscenes-and-mission-cards.md §C.0) at the moment a cinematic enters its active state. Audio subscribes per CR-CMC-11: silence-cuts to the cutscene track (audio.md §States `CUTSCENE` entry — `MusicDiegetic` ducked to −80 dB; `MusicNonDiegetic` crossfade to `cutscene` track at Music bus). Closes audio.md L407 forward-dep ("Cutscene SFX and music track swaps triggered by … custom cutscene signals (to be added to ADR-0002 during Cutscenes GDD authoring)"). Cutscenes does NOT call AudioManager directly — the signal IS the contract. Cadence: ≤ 7 cinematic activations per first-watch play-through (per cutscenes-and-mission-cards.md §C); trivially within Implementation Guideline 5 budget.

  2. **NEW signal `cutscene_ended(scene_id: StringName)`** added to Cutscenes domain. Fires from `CutscenesAndMissionCards` at the moment a cinematic exits its active state (either through natural completion or through CR-CMC-17 dismiss). Audio subscribes per CR-CMC-11: standard `music_crossfade_default_s` crossfade (2.0 s, audio.md Tuning + Crossfade Rule 6) restores section music. Mission & Level Scripting subscribes per CR-CMC-21 to write `scene_id` into `MissionState.triggers_fired` (closes Save/Load L107 + L162 forward-dep contract — replay-suppression gate consistency on `game_loaded`). MLS is the single writer to `triggers_fired`; Cutscenes does not write directly. Cadence: bounded by Cutscenes activations (1:1 with `cutscene_started`); same negligible per-frame budget impact.

  **New Cutscenes domain in Key Interfaces.** A new section `# ─── Cutscenes domain ────` is added to `Events.gd`, between Mission domain (additions) and Persistence. Currently contains exactly two signals: `cutscene_started` + `cutscene_ended`. Future cutscene-related cross-system signals (e.g., a per-card `mission_card_dismissed` event if Cutscenes & Mission Cards extends to objective-card dismissal tracking) would join this domain.

  **No new enum types**. Both signals carry `StringName` payloads only — no qualified-enum forward-reference risk like the 2026-04-22 / 2026-04-28 amendments had. The atomic-commit requirement reduces accordingly.

  **Cadence guarantees** (Implementation Guideline 5): both new signals are bounded by narrative-script activations:
  - `cutscene_started`: ≤ 7 first-watch cinematics per play-through; effectively zero per-frame budget impact.
  - `cutscene_ended`: 1:1 with `cutscene_started` (every started cutscene must end either by completion or dismiss per CR-CMC-17). Same budget profile.

  **Registry impact**: `docs/registry/architecture.yaml` `gameplay_event_dispatch.signal_signature` refreshed (**43 signals**, 2026-04-29 additions documented inline) with `revised: 2026-04-29`. New domain `cutscenes` added to the domains list. No new enum-ownership entries (StringName-only payloads).

  **Atomic-commit requirement**: This amendment's signal references are simple — `StringName`-typed payloads carry no enum forward-reference risk (per godot-gdscript-specialist validation 2026-04-29). The atomic-commit requirement reduces to: ADR-0002 amendment + `events.gd` declaration + downstream propagation in `cutscenes-and-mission-cards.md` (drop "BLOCKING ADR-0002 amendment" qualifier on CR-CMC-11) + `audio.md` L407 footnote update (drop "to be added to ADR-0002 during Cutscenes GDD authoring" forward-dep qualifier). Single-PR bundle. **No CI guard expansion needed** beyond the existing per-amendment AST check (the existing `events.gd` enum-AST guard is unaffected because no new enum reference is introduced).

  **Downstream scope flagged but out of this amendment** (producer-tracked):
  - `design/gdd/cutscenes-and-mission-cards.md` CR-CMC-11 + various downstream references — drop "BLOCKING ADR-0002 amendment" qualifier per signal-now-declared. Cutscenes-owned edit, not ADR scope.
  - `design/gdd/audio.md` §States CUTSCENE row + L407 forward-dep footnote — drop "to be added to ADR-0002 during Cutscenes GDD authoring" qualifier and reference this amendment. Audio-owned edit.
  - `design/gdd/signal-bus.md` — count 41 → 43; §54 domain table grows by Cutscenes row + AC-3 count update. Signal Bus-owned edit.
  - `design/gdd/mission-level-scripting.md` — confirm CR-CMC-21 subscriber spec for `cutscene_ended` (MLS writes `scene_id` to `triggers_fired`). MLS-owned edit (already declared per CR-CMC-21).
...
# ─── Cutscenes domain ────────────────────────────────────────────────
# Added 2026-04-29 amendment (Cutscenes & Mission Cards GDD CR-CMC-11
# BLOCKING coord — closes audio.md L407 forward-dep). Cutscenes is sole
# emitter of both signals; CutscenesAndMissionCards is a CanvasLayer 10
# lazy-instance (NOT autoload) per cutscenes-and-mission-cards.md §C.0.
# Audio subscribes to both: silence-cut to cutscene track on `_started`,
# standard 2.0 s crossfade-restore on `_ended` (audio.md §States CUTSCENE
# entry + Crossfade Rule 6). MLS subscribes to `cutscene_ended` only — to
# write `scene_id` into `MissionState.triggers_fired` (CR-CMC-21 — closes
# Save/Load L107 + L162 forward-dep + replay-suppression gate consistency
# on `game_loaded`). MLS is the SOLE writer to `triggers_fired`; Cutscenes
# does not write directly. Cadence: ≤ 7 first-watch cinematics per
# play-through (per cutscenes-and-mission-cards.md §C); 1:1 paired
# `_started`/`_ended` per cinematic; trivially within IG5 budget.
# StringName-typed payloads — no qualified-enum forward-reference risk;
# atomic-commit requirement reduces to ADR + events.gd + GDD references.
...
5. **High-frequency events**: all 43 events in this taxonomy are safe to route through the bus at their expected frequencies (per godot-gdscript-specialist analysis: weapon_fired at full-auto rate × 4 subscribers ≈ 0.02 ms/frame; `player_footstep` peaks at ~3.5 Hz during Sprint × typical 2–3 subscribers ≈ negligible; `gadget_activation_rejected` and `weapon_dry_fire_click` added 2026-04-24 are bounded by player input rate and trivially within budget; `settings_loaded` added 2026-04-28 is one-shot per session; `ui_context_changed` added 2026-04-28 is bounded by player input rate at ~2 Hz worst case during the 30-second core loop; `cutscene_started` and `cutscene_ended` added 2026-04-29 are bounded by Cutscenes activations at ≤ 7 first-watch cinematics per play-through, 1:1 paired). No event in this taxonomy is per-physics-frame.
...
| 2026-04-29 amendment introduces `cutscene_started(scene_id: StringName)` + `cutscene_ended(scene_id: StringName)` — both `StringName`-only payloads. **No qualified-enum forward-reference risk** (unlike the 2026-04-22 / 2026-04-28 risk rows above). Atomic-commit requirement reduces to single-PR bundle: ADR-0002 amendment + `events.gd` declaration + `cutscenes-and-mission-cards.md` BLOCKING-qualifier removal + `audio.md` L407 forward-dep footnote update. The existing CI guard (AST check on enum literals in `events.gd`) is unaffected — no new enum reference is introduced. | LOW | LOW | Single-PR bundle as listed; no CI guard expansion needed. (godot-gdscript-specialist validation 2026-04-29 confirmed PASS on all 5 review points.) |
...
1. Create `res://src/core/signal_bus/events.gd` with the 43 typed signal declarations.
...
- [ ] All 43 typed signals declared with qualified enum-type parameters (where applicable).
...
| `design/gdd/cutscenes-and-mission-cards.md` | Cutscenes & Mission Cards (system 22) | CR-CMC-11: "Cutscenes does not call AudioManager directly — the new `cutscene_started(scene_id: StringName)` signal drives AudioManager state" + CR-CMC-21: "MLS writes to `triggers_fired` when a `cutscene_ended(scene_id)` signal is received" | `cutscene_started` and `cutscene_ended` signals declared in Cutscenes domain (added 2026-04-29 amendment). Audio subscribes to both per audio.md §States CUTSCENE entry (silence-cut on start; 2.0 s crossfade-restore on end). MLS subscribes to `cutscene_ended` to write `scene_id` into `MissionState.triggers_fired` (replay-suppression gate consistency). Cutscenes holds no direct reference to AudioManager or MLS — the signals are the contract. |
