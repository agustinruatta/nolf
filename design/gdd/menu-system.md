...
1. **Godot engine boot.** Godot 4.6 starts; Jolt physics initialized (4.6 default); rendering initialized (Forward+ Vulkan on both Linux and Windows — D3D12 disabled via `project.godot [rendering]` per project Amendment A2 2026-04-30).
...
**Modal scaffold AccessKit descriptions** *(NEW 2026-04-29 per `design/ux/new-game-overwrite.md` + `design/ux/return-to-registry.md` + `design/ux/re-brief-operation.md` spec batch — screen-reader-only plain-language clarifications mandated by the `case-file-destructive-button` pattern rule 6 since Case File register button labels are bureaucratic-register and AT users benefit from the safety net. NOT rendered visually.)*

| tr-key | English | chars | Notes |
|---|---|---|---|
| `menu.new_game_confirm.confirm.desc` | Begin a new operation. The autosave will be overwritten and cannot be recovered. | 78 | AccessKit `accessibility_description` for `new-game-overwrite` Confirm button |
| `menu.new_game_confirm.cancel.desc` | Return to the main menu. The autosave is preserved. | 52 | AccessKit `accessibility_description` for `new-game-overwrite` Cancel button |
| `menu.return_registry.confirm.desc` | Return to the main menu. Unsaved progress in this section is lost. | 66 | AccessKit `accessibility_description` for `return-to-registry` Confirm button. Mirrors Pause Menu's Return-to-Registry button description per `pause-menu.md` L671 |
| `menu.return_registry.cancel.desc` | Resume the operation. Pause Menu remains open. | 46 | AccessKit `accessibility_description` for `return-to-registry` Cancel button |
| `menu.rebrief.confirm.desc` | Reload the last checkpoint. Recent progress since checkpoint is lost. | 69 | AccessKit `accessibility_description` for `re-brief-operation` Confirm button. Mirrors Pause Menu's Re-Brief Operation button description per `pause-menu.md` L670. **Translator briefing**: must imply *redoing the operation*, NOT *reviewing the briefing* (per `pause-menu.md` L792) |
| `menu.rebrief.cancel.desc` | Resume the operation from the current state. Pause Menu remains open. | 69 | AccessKit `accessibility_description` for `re-brief-operation` Cancel button |
