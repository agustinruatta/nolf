...
| **HUD Core** (Not Started) | outbound only | **In**: none. | **Out**: HUD Core subscribes to Inventory's 4 frozen Inventory-domain signals (`ammo_changed`, `weapon_switched`, `gadget_equipped`, `gadget_activation_rejected` *if added via ADR-0002 amendment*). HUD renders; Inventory does NOT render. | HUD Core owns rendering; Inventory owns signals. |
...
| `guard_drop_pistol_rounds` | d_p | int | **3** (Combat §F.6 registry source-of-truth per 2026-04-22 NOLF1 rebalance) | Per-lethal-kill drop. Registry entry at `design/registry/entities.yaml:854` shows stale value 8 — **flagged for Phase 5b update** |
...
| **HUD Core** (#16) | Not Started | Subscribes to `ammo_changed`, `weapon_switched`, `gadget_equipped`. If ADR-0002 amendment lands: also subscribes to `gadget_activation_rejected` (for icon desaturation) and does NOT subscribe to `weapon_dry_fire_click` (Audio-only). HUD reads `WeaponResource.magazine_size`, current magazine, and reserve via Inventory API `get_ammo(weapon_id: StringName) -> Dictionary`. HUD reads `GadgetResource.icon: Texture2D` for gadget indicator. | **Forward contract lock**: HUD Core must update in-place on rapid successive `gadget_equipped` emits (E.47 — spam gadget_next case). |
...
3. **Registry updates (Phase 5b)** — this session will handle on Inventory GDD approval:
   - `guard_drop_pistol_rounds`: **8 → 3** (stale since Combat's 2026-04-22 revision; flagged by both §D specialists 2026-04-23)
...
4. **HUD Core GDD forward-hook** (Not Started): when HUD Core GDD is authored, it must subscribe to the 4+ Inventory signals listed above and must tolerate rapid successive `gadget_equipped` emits (E.47).
...
| `guard_drop_pistol_rounds` | **3** | [2, 5] | Aggressive softlocks by Section 3 | Aggressive ammo-positive, Pillar 2 collapses | **Registry — flagged stale (8 → 3) for Phase 5b** |
