...
- [x] **Gate 3 ✅ CLOSED 2026-05-01 (Amendment A7 — PC-008)**: Logic-level closure via `tests/unit/core/player_character/player_hands_material_overlay_test.gd::test_setting_changed_resolution_scale_updates_uniform`. Emitting `Events.setting_changed(&"graphics", &"resolution_scale", 0.75)` updates `HandsOutlineMaterial.get_shader_parameter(&"resolution_scale")` to `0.75 ± 0.001` in the same call frame. The proportional visual scaling half (perceptual parity with world outline at 0.75 scale) is naturally produced by the `vertex() { VERTEX += NORMAL * outline_world_width * resolution_scale; }` shader — testing requires a real rigged mesh and is captured under Gate 4.
- [ ] **Gate 4 ⏸️ PENDING — RIGGED-MESH-DEPENDENT (PC-008 Amendment A7)**: animated rigged hand mesh (idle sway + interact reach) shows no outline artifacts at finger joints or wrist. **NOT closed in PC-008** — the rigged hand mesh + animations are post-MVP art-pipeline deliverables. PC-008 lands a placeholder `BoxMesh` HandsMesh so the structural pipeline is provable; Gate 4 closes when the rigged asset is delivered and frame-by-frame review of both animations confirms no joint/wrist artifacts. Cross-link: PC-008 Out-of-Scope section flags this dependency.
- [ ] **Gate 5 ⏸️ ADVISORY — EXPORT-DEPENDENT (PC-008 Amendment A7)**: In a Godot 4.6 export build, verify Shader Baker compiles `hands_outline_material.gdshader` assigned via `material_overlay` on the skinned hands mesh. **Procedure documented**: open project, run `Project → Export → Linux/X11`, inspect exported PCK for `hands_outline_material.gdshader.cache` (or equivalent SPIR-V cache artifact). If absent, the IG 7 two-surface fallback is required and must be documented in a follow-up amendment. **Closes** when the first export build is produced. Status remains advisory until then; any structural gap surfaces only at export time and the IG 7 fallback path is already documented in this ADR.
...
- **2026-05-01 (Amendment A7 — Gate 3 CLOSED via PC-008 implementation; Gates 4 + 5 reframed as rigged-mesh-dependent + export-dependent)**: Story PC-008 (FPS hands rendering) lands the production wiring: SubViewport at FOV 55° on PlayerCharacter Camera3D; HandsOutlineMaterial applied via `material_overlay` (NOT `material_override`); `Events.setting_changed` subscriber updates the `resolution_scale` shader uniform within one call frame. **Gate 3 ✅ closed** by Logic-level test verifying signal-driven uniform update (`tests/unit/core/player_character/player_hands_material_overlay_test.gd::test_setting_changed_resolution_scale_updates_uniform`). **Gate 4 reframed as PENDING — rigged-mesh-dependent**: PC-008 ships a placeholder `BoxMesh` HandsMesh so the pipeline is exercisable; Gate 4 closes when the rigged hand asset + idle/interact animations are delivered (post-MVP art pipeline). **Gate 5 reframed as ADVISORY — export-dependent**: full export-build Shader Baker × `material_overlay` verification deferred to first Linux export run; the IG 7 two-surface fallback procedure is documented in this ADR and ready to use if the cache artifact is missing. AC-9.1 lint guard (`tests/ci/hands_not_on_outline_tier_lint.gd`) enforces "hands MUST NOT pass through `OutlineTier.set_tier`".
