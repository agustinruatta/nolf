...
> **Status**: In Design — **NEEDS REVISION** (post-`/design-review` 2026-04-25, see `reviews/hud-core-review-log.md` for blocker list)
> **System**: #16 (Presentation / UI)
> **Tier**: MVP
> **Author**: User + `/design-system hud-core` (solo mode, 2026-04-25); revisions REV-2026-04-25 incorporated post-review
> **Last Updated**: 2026-04-25 (REV-2026-04-25 — partial revision pass; remaining items tracked in review log)
...
> **REV-2026-04-25 SCOPE NOTE**: This revision pass addresses the highest-priority blockers from the 2026-04-25 design review (24 BLOCKING items across 7 specialists). User-adjudicated design decisions: (a) TAKEDOWN_CUE demoted to OQ-HUD-7 — defaults HIDDEN until first-playtest decision; (b) photosensitivity opt-out promoted to Day-1 MVP; (c) HoH/deaf alert-state cue added to HSS forward-dep; (d) F.5 perf-budget restored via Label consolidation in V.3. Remaining recommended items tracked in review log for next-pass revision. Re-review (`/design-review design/gdd/hud-core.md` in fresh session) is BLOCKING before sprint planning.

> **Upstream (consumed)**: Player Character ✅, Combat & Damage ✅, Inventory & Gadgets ✅(pending coord), ADR-0002 Signal Bus, ADR-0004 UI Framework (Proposed — 3 gates pending), ADR-0008 Performance Budget (Slot 7 = 0.3 ms cap), Localization Scaffold (Designed)
> **Dependents (forward)**: HUD State Signaling (system #19, VS), Settings & Accessibility (system #23), Document Overlay UI (system #20 — sibling modal surface, not extension)
...
**CR-1: Signal-only subscription in `_ready()`.** HUD subscribes to all 8 frozen ADR-0002 signals plus the 2 amendment signals (see §C.5 Coord items) inside `_ready()`, using `Events.[signal].connect(_on_[signal])`. Zero subscriptions occur outside `_ready()`. The full subscription list at MVP: **(A) `Events` autoload signals (10)**: `player_health_changed`, `player_damaged`, `player_died`, `player_interacted`, `ammo_changed`, `weapon_switched`, `gadget_equipped`, `gadget_activation_rejected`, `ui_context_changed` *(NEW — ADR-0002 amendment, see §C.5)*, `takedown_availability_changed` *(NEW — ADR-0002 amendment, see §C.5)*. **(B) Settings autoload signals (2) — REV-2026-04-25**: `setting_changed("hud", "crosshair_enabled", _)`, `setting_changed("hud", "damage_flash_enabled", _)` (Day-1 photosensitivity opt-out per UI-2). **(C) Local Timer child-node signals (2) — REV-2026-04-25 explicitly listed**: `_flash_timer.timeout` → `_on_flash_timer_timeout` (CR-7b deferred-flash dispatch), `_gadget_reject_timer.timeout` → `_on_gadget_reject_timeout` (CR-9 desat revert). **(D) Viewport signal (1) — REV-2026-04-25 explicitly listed**: `get_viewport().size_changed` → `_update_hud_scale` (§C.2 scale rule). **(E) Locale signal (1)**: `setting_changed("locale", _, _)` → `_on_locale_changed` (CR-18 cache invalidation). All 16 connections are made inside `_ready()`. (Justification: ADR-0002 subscriber-only contract; ADR-0008 0.3 ms cap forbids polling; late-connect creates missed-event races.)
...
**CR-3: Two PC accessors are the only authorised polling — PC reference acquired via `@export` injection.** HUD calls `pc.get_current_interact_target()` and `pc.is_hand_busy()` once per `_process()` frame exclusively for the prompt-strip widget (§C.3). No other system property is polled. All other widget state is held in HUD-local mirror variables updated by signal handlers. **REV-2026-04-25 — PC reference acquisition**: HUD declares `@export var pc: PlayerCharacter` (typed export; `null` until injected). The main game scene (or `LevelStreamingService` on section entry / save-restore) is responsible for assigning the PC reference *before* HUD's `_ready()` returns. If `pc` is `null` at `_ready()` time, HUD logs a warning and the prompt-strip resolver returns HIDDEN until `pc` is non-null (defensive — covers integration gaps). FP-14 forbids `Engine.get_singleton(...)` and `get_tree().root.get_node(...)` lookups; the `@export` injection pattern is the spec-compliant alternative. **REV-2026-04-25 — `is_instance_valid` requirement (matches AC-HUD-6.7)**: Before any property access on the value returned by `pc.get_current_interact_target()` in `_compose_prompt_text()`, HUD MUST guard with `if is_instance_valid(target):` — a `null` check is insufficient because Godot 4.x can return a freed Object reference that passes `!= null` but fails `is_instance_valid()`. Earlier wording specified "null check only" — that was incorrect; the AC is authoritative. (Justification: ADR-0008 0.3 ms cap; ADR-0004 §Implementation Guideline 12 explicitly authorises exactly these two accessors. Adding a third would require explicit ADR amendment — which is precisely why TAKEDOWN_CUE was resolved via signal subscription, not polling. `@export` injection vs autoload lookup keeps the dependency arrow explicit and testable.)

**CR-4: Single source of truth per widget.** Each widget derives from exactly one authoritative signal:
...
- Prompt-strip resolver ← `pc.get_current_interact_target()` + `pc.is_hand_busy()` (per frame) + `takedown_availability_changed(eligible, target)` (signal latch)
- HUD parent visibility ← `ui_context_changed(new_ctx, prev_ctx)` (signal latch)
- Crosshair visibility ← HUD parent visibility AND `setting_changed("hud", "crosshair_enabled", _)` mirror
...
**CR-8: Dry-fire feedback via `ammo_changed` unchanged-value detection.** HUD MUST NOT subscribe to `weapon_dry_fire_click` (Audio's exclusive subscription per ADR-0002 amendment 2026-04-24). Instead: in `_on_ammo_changed(weapon_id, current, reserve)`, HUD compares against locally cached prior values `(_last_ammo_weapon_id, _last_ammo_current, _last_ammo_reserve)`. If all three identical → trigger 1-frame magazine-numeral flash (separate from damage flash; not photosensitivity-gated because dry-fire cadence is bounded by player input rate ≤ 30 Hz). The cache is updated on every `ammo_changed` event. (Justification: ADR-0002 amendment expressly forbids `weapon_dry_fire_click` HUD subscription; this detection is the specified workaround.)
...
**CR-11: Crosshair widget gates on parent visibility AND Settings.crosshair_enabled.** The crosshair `Control` child is visible iff `(hud_root.visible == true) AND (_crosshair_enabled_mirror == true)`. The mirror is updated on `setting_changed("hud", "crosshair_enabled", value)` from Settings & Accessibility (forward-dep on system #23). Initial value at `_ready()`: false (until Settings & Accessibility emits the initial value during its boot, which happens before the HUD scene receives input — verified at integration time, OQ-HUD-3). (Justification: Pillar 5 — crosshair is the only opt-out exception to the no-centre-chrome rule; settings toggle must take effect immediately.)

**CR-12: Prompt-strip state machine — 3 MVP states (TAKEDOWN_CUE visibility deferred per OQ-HUD-7).** Prompt-strip is in exactly one of three states at any instant: `HIDDEN`, `INTERACT_PROMPT`, `TAKEDOWN_CUE`. Only one renders. Priority highest wins: `TAKEDOWN_CUE` > `INTERACT_PROMPT` > `HIDDEN`. Resolver evaluates each `_process()` frame from the PC queries and the `_takedown_eligible: bool` latch (updated by `takedown_availability_changed`). Full transition logic: §C.3. **REV-2026-04-25 — TAKEDOWN_CUE rendered text is deferred to OQ-HUD-7** (game-designer + creative-director flagged the literal "TAKEDOWN AVAILABLE" string as a Pillar-5/cockpit-dial-fantasy violation — the HUD should confirm, not invite). At MVP, `_takedown_eligible` is *captured and updated* by `takedown_availability_changed` so the latch is implementation-ready, but the resolver default returns `HIDDEN` for the `TAKEDOWN_CUE` branch (visible text suppressed) until OQ-HUD-7 closes via first-playtest decision. **MEMO_NOTIFICATION is deliberately deferred to HUD State Signaling (system #19, VS)** — HUD Core MVP scope does not include pickup-toast lifecycle. (Justification: HUD State Signaling owns "alarm indicator + pickup notifications" per its systems-index entry; deferring closes OQ-HUD-2 cleanly.)

**CR-13: INTERACT_PROMPT suppressed during `pc.is_hand_busy()` window; TAKEDOWN_CUE is NOT.** When `pc.is_hand_busy()` returns `true` (PC's pre-reach + reach window), the `INTERACT_PROMPT` state is suppressed even if `pc.get_current_interact_target()` returns non-null. `TAKEDOWN_CUE` is **not** suppressed — a takedown-eligible target entering range during a hand-busy window still renders the cue. (Justification: PC §UI Requirements specifies `is_hand_busy()` suppresses interact prompt; takedown cues are state facts, not invitations to act — suppressing them would hide actionable information.)
...
| **Prompt-strip** | `ANCHOR_PRESET_CENTER_BOTTOM` | y-offset −18% from bottom-edge / horizontally centered / size auto @ 14 px font | Single `Label` (Futura Condensed Bold, 14 px, Parchment on BQA Blue strip — uses StyleBoxFlat from `hud_theme.tres`) | 3 | Visible only in non-HIDDEN state (§C.3 state machine) | `pc.get_current_interact_target()` (poll), `pc.is_hand_busy()` (poll), `takedown_availability_changed` (latch) |
| **Crosshair** | `ANCHOR_PRESET_CENTER` | viewport center | `Control` subclass with `_draw()` override (`draw_circle(center, dot_radius)` + tri-band halo via `draw_arc`) | 4 (front of HUD widgets, but below ADR-0004 §7 layer 4 sepia dim) | Visible iff parent.visible AND `_crosshair_enabled_mirror` (CR-11) | `setting_changed("hud", "crosshair_enabled", _)` |

**Scale rule.** HUD root's `Control` child has `anchors_preset = ANCHOR_PRESET_FULL_RECT`. All widget anchors are preset-based (corners + center) — anchor presets in Godot 4.6 maintain corner/center positioning correctly across resolution changes without explicit scale math. **Pixel offsets (margins) are the design values at 1080p reference; viewport-height scaling is handled by setting each widget Control's `scale = Vector2(1, 1) * (viewport.size.y / 1080.0)` once at `_ready()` and again on `viewport.size_changed`** (single signal subscription, not per-frame). Widget contents (font sizes, padding) are sized in *design pixels*; the parent scale propagates. **Ultrawide (21:9 / 32:9) clamping**: corner widgets stay anchored to corners (always inside safe area); the prompt-strip's `ANCHOR_PRESET_CENTER_BOTTOM` keeps it centered horizontally regardless of aspect, so 21:9/32:9 widen the playable area without affecting prompt visibility. **No HUD-scale slider at MVP** — Settings & Accessibility may add one as a forward-dep (OQ-HUD-1).

### C.3 Prompt-Strip State Machine

| State | Renders | Trigger / latch source |
|---|---|---|
| `HIDDEN` | Nothing (Label `visible = false`) | Default; no eligible state |
| `INTERACT_PROMPT` | `tr("HUD_INTERACT_PROMPT")` followed by `tr(target.interact_label_key)` (e.g., "PRESS [E] TO LIFT COVER") | `pc.get_current_interact_target()` returns non-null AND `!pc.is_hand_busy()` (CR-13) |
| `TAKEDOWN_CUE` | `tr("HUD_TAKEDOWN_AVAILABLE")` ("TAKEDOWN AVAILABLE") | `_takedown_eligible == true` (latch updated by `takedown_availability_changed(eligible, target)`) |

**Transition resolver.** Each `_process(_delta)` frame, HUD evaluates priority order (highest wins):
```
# REV-2026-04-25: TAKEDOWN_CUE branch returns HIDDEN per OQ-HUD-7 default (Path A).
# When OQ-HUD-7 closes to Path B/C, restore the TAKEDOWN_CUE return value.
if _takedown_eligible:
    state := HIDDEN  # OQ-HUD-7 Path A — was: TAKEDOWN_CUE
...
**Resolver writes to `Label.text` only when state OR target changes** (string-equality guard per godot-specialist Item 3 — `Label.text =` invalidates TextServer layout). Pseudocode:
```gdscript
var new_state := _resolve_prompt_state()
var new_text := _compose_prompt_text(new_state, current_target)
if new_state != _last_state or new_text != _label.text:
...
**No auto-dismiss timer at MVP.** Without MEMO_NOTIFICATION, no transient state requires auto-dismissal. INTERACT_PROMPT and TAKEDOWN_CUE are both *latch-driven* — they hide as soon as their underlying condition is false.
...
| **Stealth AI ✅** | inbound (signal — NEW) | Subscribes: `takedown_availability_changed(eligible: bool, target: Node3D \| null)` *(NEW — ADR-0002 amendment, see Coord item §C.5#2)*. Drives TAKEDOWN_CUE state (CR-12). |
...
| **Settings & Accessibility (system #23, Not Started)** | inbound (signal forward-dep) | Subscribes: `setting_changed("hud", "crosshair_enabled", value)` (and similar for `crosshair_dot_size_pct_v`, `crosshair_halo_style` once Settings GDD authored). **Forward dependency.** |
...
| **HUD State Signaling (system #19, VS)** | outbound (extension) | HUD State Signaling extends HUD Core's prompt-strip with MEMO_NOTIFICATION and the alarm-state stinger; subscribes to `document_collected`, `alert_state_changed (severity)`, `respawn_triggered`. **HUD Core MVP defines the `_label: Label` child node and exposes a public method `get_prompt_label() -> Label` for HSS consumption** (single forward extension point). |
...
| **ADR-0002 Signal Bus** | inbound (subscriber) | HUD subscribes to 10 signals (8 frozen + 2 amendment). Emits zero. |
...
1. **ADR-0002 amendment**: add `signal ui_context_changed(new_context: InputContext.Context, previous_context: InputContext.Context)` to UI domain. **REV-2026-04-25** — argument type uses the autoload-key form `InputContext.Context` (not the class name `InputContextStack.Context`) per ADR-0004 Implementation Guideline 2. Emitter: `InputContext` autoload at end of `push()` and `pop()`. Subscribers (initial): HUD Core (CR-10). (Owner: tech-lead via `/architecture-decision adr-0002-amendment` bundle.)
2. **ADR-0002 amendment**: add `signal takedown_availability_changed(eligible: bool, target: Node3D)` to Stealth AI domain. Emitter: SAI when its `takedown_prompt_active` accessor flips. Subscribers (initial): HUD Core (CR-12). Coordinated bundle with #1. (Owner: tech-lead.)
...
**FP-13: No HUD as autoload.** Pattern: any HUD path appearing in `project.godot` `[autoload]` section — HUD is per-main-scene `CanvasLayer`, not autoload; ADR-0007 caps autoloads at 7 (Combat took the last slot).
...
### F.5 — HUD Per-Frame Cost Composition **REV-2026-04-25 — formula expanded; constants flagged as unmeasured pending Iris Xe / Godot 4.6 verification (see OQ-HUD-5)**

The hud_frame_cost formula is defined as:

`C_frame = C_draw + C_poll + C_label × N_label_updates + C_flash × flash_active + C_resize × resize_events_this_frame + C_theme_override × theme_override_writes`

**Variables:**

| Variable | Symbol | Type | Range (estimated, **UNMEASURED on Godot 4.6 — see OQ-HUD-5**) | Description |
|----------|--------|------|-------|-------------|
| Crosshair `_draw()` cost | `C_draw` | float (ms) | **estimate [0.005, 0.020]** | Cost of 2 × 64-segment `draw_arc` + `draw_circle` per GAMEPLAY frame (§V.6). **Always runs while crosshair visible**. Zero when crosshair disabled or HUD invisible. **REV-2026-04-25**: this term was missing from the prior formula. |
| Interact-prompt poll cost | `C_poll` | float (ms) | estimate [0.002, 0.004] | Fixed per-frame: `get_current_interact_target()` + `is_hand_busy()` + state eval + change-guard. Always runs. |
| Per-Label update cost | `C_label` | float (ms) | estimate [0.02, 0.05] **UNMEASURED on Godot 4.6** | Cost of a single `Label.text` write (TextServer invalidation). Zero on frames with no signal emissions. TextServer was reworked in Godot 4.4–4.5; the upper-bound estimate may be low if Forward+ Mobile renderer applies additional overhead. **OQ-HUD-5 BLOCKING gate**: must be measured on Iris Xe Gen 12 / 4.6 / 810p before sprint-plan closes. |
| Number of Label updates this frame | `N_label_updates` | int | [0, **5**] **REV-2026-04-25** | Count of Labels written this frame: health (1), consolidated ammo (1) — *was 3*, weapon-name (1), gadget-slot (1), prompt-strip (1) — *was missing in prior formula*. **Net change: −1 from Label consolidation, +1 from prompt-strip inclusion = same N=5 maximum**. 0 on idle frames. |
| Flash deferred-frame cost | `C_flash` | float (ms) | estimate [0.001, 0.005] | Cost of one `await process_frame` + revert `add_theme_color_override` write. Active at most 3 Hz (≤ 1 frame per 333 ms). |
| Flash active this frame | `flash_active` | bool (0 or 1) | {0, 1} | 1 on the frame the flash fires; 0 otherwise. |
| Resize handler cost | `C_resize` | float (ms) | estimate [0.001, 0.003] **REV-2026-04-25 — added** | Cost of `_update_hud_scale()` per `viewport.size_changed` emission: read `viewport.size.y`, write `Control.scale`. **Layout invalidation cascade fires on the subsequent frame**, not in this term — see Cluster H for the cross-frame cost. |
| Resize events this frame | `resize_events_this_frame` | int | [0, ~30] | Number of `viewport.size_changed` emissions this frame. Typically 0; up to ~30 during a window-resize drag. |
| Theme-override write cost | `C_theme_override` | float (ms) | estimate [0.005, 0.015] **REV-2026-04-25 — added** | Cost of one `add_theme_color_override(&"font_color", color)` call: hash-map insert + `NOTIFICATION_THEME_CHANGED` propagation down the Label subtree. Active on critical-state edge transitions (CR-5/CR-6) and on flash start/revert frames. |
| Theme-override writes this frame | `theme_override_writes` | int | [0, 4] | Maximum: critical-state edge (1) + flash start (1) + flash revert next frame (1, but on a different frame) + a defensive 1 for a rapid critical-recovery edge. |
| Total frame cost | `C_frame` | float (ms) | estimate [0.005, ~0.30] **UNMEASURED** | HUD's contribution to ADR-0008 Slot 7 (cap = 0.3 ms). |

**Output Range (estimated, unverified):**

- **Normal idle (crosshair on, no signals)**: `C_draw + C_poll ≈ 0.013 + 0.003 = ~0.016 ms` (5% of cap).
- **Typical combat frame** (1 ammo-change, no flash, no critical edge, no resize): `C_draw + C_poll + 1 × C_label = 0.013 + 0.003 + 0.035 = ~0.051 ms`.
- **Flash frame** (1 ammo + flash + critical edge): `C_draw + C_poll + 2 × C_label + C_flash + 2 × C_theme_override = 0.013 + 0.003 + 0.07 + 0.003 + 0.020 = ~0.109 ms`.
- **Worst-case single exceptional frame** (5 simultaneous events: multi-hit flash + ammo update + weapon update + gadget update + critical-state colour swap + prompt-strip update): `C_draw + C_poll + 5 × C_label_max + C_flash_max + 2 × C_theme_override_max = 0.020 + 0.004 + 5 × 0.050 + 0.005 + 0.030 = ~0.309 ms` **— this is at-or-over the 0.3 ms Slot 7 cap depending on measured `C_label`. REV-2026-04-25 ammo-Label consolidation lowers the more-realistic worst case (no `C_theme_override` peak) to ~0.279 ms with ~21 µs headroom — but this is still thin and constants are unmeasured.**

**OQ-HUD-5 BLOCKING gate (REV-2026-04-25 — escalated)**: `C_label`, `C_draw`, `C_poll`, `C_theme_override` MUST be measured on Iris Xe Gen 12 / Godot 4.6 / Forward+ Mobile / 810p Restaurant reference scene before sprint-plan closes. If measured worst-case exceeds 0.3 ms, options: (a) ADR-0008 Slot 7 amendment to 0.31 ms; (b) further Label consolidation (merge "HP" sub-label into health numeral); (c) pre-render crosshair to `Texture2D` (eliminates `C_draw`). See OQ-HUD-5 for full resolution path.

**Resize-drag is excluded from steady-state worst case**: `viewport.size_changed` only fires during window-resize drag, which is mutually exclusive with active gameplay (no Restaurant-scene combat scenario includes a live resize). The CI gate for AC-HUD-9.x explicitly excludes the resize term — it is documented in the formula for completeness but not measured.

### Why dry-fire flash is NOT rate-gated

The dry-fire flash (CR-8) is triggered by player input (the `Fire` action), processed once per `_input` callback — one flash per input event, bounded by held-key repeat rate (typically 30 Hz on keyboard, 60 Hz on gamepad). This is two orders of magnitude below WCAG 2.3.1's 3 Hz threshold concern. More importantly, dry-fire is a *deliberate, player-controlled* action: the player decides to keep pulling the trigger. A photosensitivity rate-gate on deliberate player actions would be paternalistic and break the expected immediate feedback loop. The WCAG rate-gate on `player_damaged` is warranted precisely because *incoming damage is not player-controlled* and can arrive in volleys outside the player's timing authority. No formula needed; CR-8's "cadence bounded by player input rate" is sufficient.
...
**If `pc.get_current_interact_target()` returns null while `pc.is_hand_busy()` is true**: Resolver evaluates: `_takedown_eligible == false` → check INTERACT_PROMPT → `get_current_interact_target() == null` → `HIDDEN`. The `is_hand_busy()` check is irrelevant because `get_current_interact_target()` already returned null; the null check short-circuits before `is_hand_busy()` is evaluated in the INTERACT_PROMPT branch (CR-12 resolver logic). HUD shows HIDDEN. Correct — player already lost the target.

**If `takedown_availability_changed(true, target)` fires but the target `Node3D` is freed before the next `_process` frame**: HUD stores `_takedown_eligible = true` (a bool) — it does NOT store the `target` Node3D reference. The TAKEDOWN_CUE string `tr("HUD_TAKEDOWN_AVAILABLE")` renders without referencing the target. No `is_instance_valid` check needed by HUD because it never holds the reference. The CUE persists until `takedown_availability_changed(false, null)` fires (SAI is responsible for emitting this when the target becomes invalid). **If SAI fails to emit the clearing signal after target death, the stale TAKEDOWN_CUE renders until the next section transition** — a SAI defect, not a HUD defect. The HUD AC for this case must verify SAI emits `takedown_availability_changed(false, null)` on guard death.

**If INTERACT_PROMPT and TAKEDOWN_CUE are both eligible simultaneously**: CR-12 priority rule applied deterministically: TAKEDOWN_CUE (highest priority) wins. INTERACT_PROMPT is not rendered. On the frame the TAKEDOWN_CUE condition clears (SAI emits `takedown_availability_changed(false, null)` → `_takedown_eligible = false`), the resolver re-evaluates: if `get_current_interact_target() != null` and `!is_hand_busy()` → transitions to INTERACT_PROMPT on that same frame. No intermediate HIDDEN frame occurs because both conditions are evaluated in the same `_process` call.
...
**If the HUD scene is freed and re-instantiated on `LOAD_FROM_SAVE`**: Per CR-14, `_ready()` initialises all widget values to zero/empty defaults. `_flash_timer` is stopped, `_pending_flash = false`, `_was_critical = false`, `_takedown_eligible = false`. LSS restore-callback sequence then re-emits `player_health_changed`, `ammo_changed`, `weapon_switched`, `gadget_equipped`, `ui_context_changed` with restored values. HUD receives these identically to live gameplay and populates widgets. No special-case path; signal replay is the full restore contract.
...
**If `setting_changed("hud", "crosshair_enabled", false)` fires while the crosshair Control is mid-`_draw()` call**: Godot 4.x `_draw()` runs as part of the render server's draw call, after `_process`. The `setting_changed` signal is processed in `_process` time, updating `_crosshair_enabled_mirror = false` and calling `crosshair.visible = false`. The visibility change is applied before the next `_draw()` dispatch — the next frame the crosshair does not draw. There is no mid-draw interrupt. The current frame's crosshair draw completes normally; visibility takes effect on the next frame. Single-frame crosshair artefact on the toggle frame is acceptable.
...
| **Stealth AI** ✅ | Approved | HARD | TAKEDOWN_CUE prompt-strip state requires `takedown_availability_changed` signal (NEW — SAI domain ADR-0002 amendment). |
...
| **Settings & Accessibility** (system #23) | Not Started | Must define `crosshair_enabled: bool`, `crosshair_dot_size_pct_v: float`, `crosshair_halo_style: enum {none, parchment_only, tri_band}`. Must emit `setting_changed("hud", _, _)` per ADR-0002. Must support locale-change re-emission for `tr()` invalidation per CR-18. |
...
| **ADR-0002 Signal Bus + Event Taxonomy** | Proposed | HUD subscribes to 10 signals (8 frozen + 2 amendment). Emits zero. |
...
### Coordination items summary (rolled up from §C.5)

**BLOCKING (5+)** — sprint cannot start until these close:
1. ADR-0002 amendment: `ui_context_changed(new_ctx, prev_ctx)` (UI domain) bundle with item 2 — argument type is `InputContext.Context` (autoload-key form per REV-2026-04-25)
2. ADR-0002 amendment: `takedown_availability_changed(eligible, target)` (SAI domain) bundle with item 1
3. ADR-0004 Gate 2: confirm Theme inheritance property name (`base_theme` vs `fallback_theme`)
4. ADR-0004 Gate 1: confirm `accessibility_live` property name on Godot 4.6 Label/Control (REV-2026-04-25 promoted to BLOCKING-before-MVP for spec correctness, even if AccessKit feature is Polish-tier)
5. **REV-2026-04-25 — Godot 4.6 API verification batch** (see §C.5 Coord item #5): `Color(hex,alpha)` constructor, `TextureRect.EXPAND_FIT_WIDTH_PROPORTIONAL` enum, `set_anchors_preset()` vs property, `Performance.TIME_PROCESS` constant, `process_frame` signal name, `font_color` theme key, `focus_mode = FOCUS_NONE` default, `corner_radius_*` property names. ~30 min batch verification.
6. **REV-2026-04-25 — OQ-HUD-5 escalated**: F.5 constants (`C_label`, `C_draw`, `C_poll`, `C_theme_override`) MUST be measured on Iris Xe Gen 12 / Godot 4.6 / 810p Restaurant before sprint-plan closes (was advisory contingency).

**ADVISORY (3)**:
5. Settings & Accessibility GDD when authored — define crosshair settings contract per §C.5 row
6. HUD-scale slider as Settings forward-dep (OQ-HUD-1)
...
| SAI `takedown_availability_changed` (NEW signal) | SAI must add this signal — Coord item §C.5#2 ⏳ | ⏳ pending amendment |
...
| HSS forward-extension via `get_prompt_label()` | HSS not yet authored — provisional contract | ⏳ pending HSS GDD |
| Settings & Accessibility crosshair settings | Settings GDD not yet authored — forward-dep | ⏳ pending Settings GDD |
...
| `hud_damage_flash_enabled` (true default) | **REV-2026-04-25 — Day-1 MVP requirement** (was: Settings & Accessibility forward-dep) | Photosensitivity opt-out toggle. When false, the F.1 rate-gate is bypassed and `_pending_flash` is never set; visual flash is fully suppressed. **Day-1 MVP via stub `Settings.get_setting("hud", "damage_flash_enabled", true)` accessor**. Settings & Accessibility GDD authoring later wires up persistence; HUD Core depends on the contract not the implementation. Industry/cert/legal floor — see UI-2 row. |
...
PanelContainer                    [StyleBoxFlat: gadget_tile_tint_base — #2A4F8A fully opaque]
  PanelContainer                  [StyleBoxFlat: gadget_tile_overlay — #1B3A6B 85%]
    Control                       [custom _draw() — no additional draw; children handle content]
      TextureRect(_icon_texture)  [expand_mode: FIT_WIDTH_PROPORTIONAL, stretch_mode: KEEP_ASPECT_CENTERED, size 56×56]
...
**`_current_health_color` revert rules — REV-2026-04-25 with re-entry + freed-self guards:** the revert colour is captured at the moment the flash begins — before the `await` suspends the coroutine; the `_flashing` re-entry flag prevents concurrent coroutines; an `is_instance_valid(self)` check after `await` covers the freed-during-await case:
...
func _execute_flash() -> void:
    ...
    if not is_instance_valid(self):  # HUD was freed mid-flash — bail out
        return
...
**Prompt-strip transitions — silent.** Prompt appearing (`HIDDEN → INTERACT_PROMPT` or `HIDDEN → TAKEDOWN_CUE`) and dismissing produce no audio event. There is no UI click, no pop sound, no confirmation chime. The prompt is a contextual peripheral readout, not an interactive element.
...
| **Photosensitivity opt-out toggle** | **Day 1** **(REV-2026-04-25 — promoted from forward-dep)** | `hud_damage_flash_enabled: bool` (default `true`). When `false`, F.1 rate-gate is bypassed and `_pending_flash` is never set — visual flash is fully suppressed. HUD Core MVP reads via `Settings.get_setting("hud", "damage_flash_enabled", true)` (stub accessor returning the default if Settings autoload has not booted; Settings & Accessibility GDD authoring wires the persistence layer). Boot-screen photosensitivity warning (system #22) MUST direct players to this toggle Day 1. Industry/cert/legal floor — Sony, Microsoft, EU GAAD all expect Day-1 photosensitivity opt-outs. The rate-gate is NOT a substitute for opt-out. |
| **HoH/Deaf accessibility — alert-state cue** | **Day 1 — surface owned by HSS (forward-dep)** **(REV-2026-04-25 — NEW)** | Pillar 5 forbids visual alert-state indicators in HUD Core. To close the WCAG 1.1.1 / 1.3.3 + EU GAAD floor for hard-of-hearing and deaf players, **HUD State Signaling (system #19, VS) MUST surface a brief text-only alert-state cue** (e.g., `tr("HUD_GUARD_ALERTED")`) that fires once on alert-state entry and auto-dismisses ~2 s. The cue uses HUD Core's prompt-strip Label via `get_prompt_label()` (UI-3 extension API). No persistent meter, no directional indicator, no suspicion bar — period-typographic, momentary, Pillar-5-compatible. **Forward-dep BLOCKING for VS**: HSS GDD authoring must include this AC. Day-1 ship without HSS is NOT compliant; if HSS slips, document a formal accessibility exception with named owner + post-launch resolution timeline. |
| Reduced-motion mode | **Day 1** **(REV-2026-04-25 — corrected)** | Rate-gate (F.1, 3 Hz ceiling) is the Day-1 reduced-motion accommodation; full opt-out is the photosensitivity toggle row above. Earlier "no motion to reduce" claim was incorrect — luminance changes in the damage flash ARE motion stimuli. The toggle is the single user-facing control covering both photosensitivity and reduced-motion needs. |
...
**AC-HUD-1.1 [Integration] [BLOCKING]**: GIVEN the HUD Core scene is loaded and `Events` autoload is initialised per ADR-0007, WHEN HUD `_ready()` completes, THEN all 10 signals (`player_health_changed`, `player_damaged`, `player_died`, `player_interacted`, `ammo_changed`, `weapon_switched`, `gadget_equipped`, `gadget_activation_rejected`, `ui_context_changed`, `takedown_availability_changed`) have exactly one connection each to their respective handlers, verified via `Events.[signal].is_connected(handler) == true` for all 10. Evidence: `tests/integration/hud_core/test_subscription_lifecycle.gd`

**AC-HUD-1.2 [Integration] [BLOCKING]**: GIVEN HUD Core is connected with all 10 signals, WHEN `_exit_tree()` is called, THEN all 10 signals are disconnected and `Events.[signal].is_connected(handler) == false` for each, with no GDScript error emitted from a missing-connection guard. Evidence: `tests/integration/hud_core/test_subscription_lifecycle.gd`
...
**AC-HUD-1.5 [Integration] [BLOCKING]** **REV-2026-04-25 — argument order corrected**: GIVEN a `LOAD_FROM_SAVE` trigger that frees and re-instantiates the HUD scene, WHEN the new HUD instance's `_ready()` completes and LSS re-emits `player_health_changed(50, 100)`, `ammo_changed("walther_ppk", 6, 21)`, `weapon_switched("walther_ppk")`, `gadget_equipped("cigarette_case")`, `ui_context_changed(InputContext.Context.GAMEPLAY, InputContext.Context.MENU)` *(args: new_ctx=GAMEPLAY, prev_ctx=MENU — i.e. transitioning INTO gameplay, which is the visible-HUD state per AC-HUD-8.1)*, THEN all five widgets reflect the replayed values, `hud_root.visible == true`, and all 10 `Events`-bus signals are connected exactly once in the new instance. Evidence: `tests/integration/hud_core/test_save_load_lifecycle.gd`
...
**AC-HUD-2.7 [UI] [ADVISORY]** **REV-2026-04-25 — deterministic gate added**: GIVEN the HUD is rendered in the Restaurant reference scene with Alarm Orange active (`_was_critical = true`), WHEN a screenshot is taken, THEN Alarm Orange `#E85D2A` achieves a minimum luminance contrast ratio of **3.0:1** (WCAG 2.1 AA non-text threshold) against the sampled restaurant wall colour at the health widget position, AND a colorblind simulation (deuteranopia, protanopia, tritanopia minimum) preserves distinguishability of the colour state from non-critical Parchment `#F2E8C8`. Contrast ratio measured with a pixel-sampler tool (e.g., the `colour-contrast.dev` CLI or equivalent) and recorded numerically in the evidence file. Art Director sign-off remains a courtesy gate but is no longer required for AC pass — contrast measurement + colorblind simulation evidence alone constitute pass evidence. If Art Director is unavailable, lead programmer confirms the screenshot matches the spec. Evidence: `production/qa/evidence/hud_core/screenshot_critical_health_restaurant_<date>.png` + `contrast_<date>.txt` (raw measurement).

### H.3 — Photosensitivity Rate-Gate (F.1)

**AC-HUD-3.1 [Logic] [BLOCKING]** **REV-2026-04-25 — timer mechanism specified**: GIVEN `_flash_timer` is stopped, WHEN `player_damaged` is emitted at simulated test-harness times `t=0 ms`, `t=150 ms`, and `t=250 ms` (advanced via `_flash_timer.timeout.emit()` for the timeout boundary OR via `gut.simulate(node, frames, delta)` to step time without real-time `await`; **real-time `await get_tree().create_timer(...)` is NOT permitted** — tests must be deterministic), THEN exactly 2 flash events fire: one immediate at `t=0` (gate opens, timer starts) and one deferred at `t=333 ms` (timer timeout fires, `_pending_flash` was `true`). The 3rd hit at `t=250 ms` produces no additional flash output. Verified by a signal-spy on `_execute_flash` counting 2 invocations total. Evidence: `tests/unit/hud_core/test_damage_flash.gd`

**AC-HUD-3.2 [Logic] [BLOCKING]** **REV-2026-04-25 — boundary disambiguated**: GIVEN 100 `player_damaged` signals are emitted in a single test frame via `for i in 100: Events.player_damaged.emit(...)` (no `await` between iterations — fully synchronous loop), WHEN the loop completes, THEN exactly 1 immediate flash has fired (iteration 1 found `is_stopped() == true`), `_pending_flash == true`, `_flash_timer.is_stopped() == false`, and `_flash_timer.time_left == 0.333` *(the timer was started at iteration 1 and no simulated time has elapsed within the synchronous loop — `time_left` equals `wait_time` exactly, not `≤`)*. After advancing simulated time to the timeout boundary, exactly 1 deferred flash fires and `_pending_flash` resets to `false`. Total flash output: 2. Evidence: `tests/unit/hud_core/test_damage_flash.gd`
...
**AC-HUD-3.7 [Logic] [BLOCKING]** **REV-2026-04-25 — NEW (sustained dry-fire WCAG safety)**: GIVEN cached ammo state is `("walther_ppk", 0, 0)` and `Settings.get_setting("hud", "damage_flash_enabled", true) == true`, WHEN `ammo_changed("walther_ppk", 0, 0)` is emitted 60 consecutive times across 1 second of simulated input (held trigger with empty magazine — dry-fire pattern at 60 Hz keyboard repeat rate), THEN the consolidated ammo Label dry-fire flash fires at most **3 times in that 1-second window** — i.e., the dry-fire flash is rate-gated to the same WCAG 2.3.1 ceiling (3 Hz) as the damage flash. The CR-8 spec previously asserted dry-fire flash was un-gated because "input rate is bounded by player input"; held-trigger keyboard repeat at 30-60 Hz violates this assumption. **REV-2026-04-25 implementation note**: route dry-fire detection through the same `_flash_timer` as damage flash, OR add a dedicated `_dry_fire_timer` Timer child with `wait_time = 0.333`. Either path is acceptable; both prevent WCAG 2.3.1 violation under sustained input. Evidence: `tests/unit/hud_core/test_weapon_ammo_widget.gd`

**AC-HUD-3.8 [Logic] [BLOCKING]** **REV-2026-04-25 — NEW (Day-1 photosensitivity opt-out)**: GIVEN `Settings.get_setting("hud", "damage_flash_enabled", true) == false`, WHEN `player_damaged(amount, source, is_critical)` is emitted, THEN no visual flash fires — `_flashing` remains `false`, `_pending_flash` remains `false`, `_flash_timer` remains stopped, and the health Label's `font_color` override is unchanged. Audio SFX handler still fires independently (same `player_damaged` signal — see AC-HUD-3.6). When the toggle flips back to `true`, the next `player_damaged` event resumes normal F.1 rate-gated behaviour. Evidence: `tests/integration/hud_core/test_photosensitivity_optout.gd`
...
**AC-HUD-4.5 [Logic] [BLOCKING]** **REV-2026-04-25**: GIVEN cached ammo state is `("walther_ppk", 0, 0)`, WHEN `ammo_changed("walther_ppk", 0, 0)` is emitted (unchanged values — dry-fire pattern), THEN the consolidated ammo Label triggers a 1-frame magazine-numeral flash distinct from the damage flash, verified by a spy on the dry-fire flash path executing once. This flash is NOT gated by `_flash_timer`. Evidence: `tests/unit/hud_core/test_weapon_ammo_widget.gd`
...
**AC-HUD-6.1 [Logic] [BLOCKING]**: GIVEN `_takedown_eligible = false` and `pc.get_current_interact_target()` returns `null`, WHEN `_process` evaluates the resolver, THEN the prompt-strip `Label.visible == false` and prompt state is `HIDDEN`. Evidence: `tests/unit/hud_core/test_prompt_strip_state_machine.gd`

**AC-HUD-6.2 [Logic] [BLOCKING]**: GIVEN `_takedown_eligible = false`, `pc.get_current_interact_target()` returns a non-null stub target, and `pc.is_hand_busy()` returns `false`, WHEN `_process` evaluates the resolver, THEN the prompt-strip `Label.visible == true` and `Label.text` equals the composed `INTERACT_PROMPT` string. Evidence: `tests/unit/hud_core/test_prompt_strip_state_machine.gd`

**AC-HUD-6.3 [Logic] [BLOCKING]**: GIVEN `_takedown_eligible = true` (set by a prior `takedown_availability_changed(true, stub_target)` emission), regardless of the values of `pc.get_current_interact_target()` and `pc.is_hand_busy()`, WHEN `_process` evaluates the resolver, THEN the prompt-strip `Label.text` equals `tr("HUD_TAKEDOWN_AVAILABLE")` and `TAKEDOWN_CUE` takes priority over any INTERACT_PROMPT candidate. Evidence: `tests/unit/hud_core/test_prompt_strip_state_machine.gd`

**AC-HUD-6.4 [Logic] [BLOCKING]**: GIVEN `_takedown_eligible = true` AND a non-null interact target exists AND `pc.is_hand_busy()` returns `true`, WHEN `_process` evaluates the resolver, THEN the prompt-strip shows `TAKEDOWN_CUE` (not `INTERACT_PROMPT` and not `HIDDEN`) — TAKEDOWN_CUE is NOT suppressed by `is_hand_busy()`. Evidence: `tests/unit/hud_core/test_prompt_strip_state_machine.gd`

**AC-HUD-6.5 [Logic] [BLOCKING]**: GIVEN `_takedown_eligible = false`, `pc.get_current_interact_target()` returns a non-null target, and `pc.is_hand_busy()` returns `true`, WHEN `_process` evaluates the resolver, THEN the prompt-strip `Label.visible == false` (INTERACT_PROMPT is suppressed during hand-busy window). Evidence: `tests/unit/hud_core/test_prompt_strip_state_machine.gd`
...
**AC-HUD-6.7 [Logic] [BLOCKING]**: GIVEN `pc.get_current_interact_target()` returns a valid stub target that is then freed between physics frames (simulated via `queue_free()` on the stub), WHEN `_compose_prompt_text()` is called, THEN `is_instance_valid(target)` is checked before any property access on `target`, and no null-deref crash occurs. Evidence: `tests/unit/hud_core/test_prompt_strip_state_machine.gd`
...
**AC-HUD-7.4 [Logic] [BLOCKING]**: GIVEN `_crosshair_enabled_mirror = true`, WHEN `setting_changed("hud", "crosshair_enabled", false)` is emitted, THEN `_crosshair_enabled_mirror == false` and `crosshair.visible == false` within the same frame (no deferred update). WHEN `setting_changed("hud", "crosshair_enabled", true)` is emitted, THEN `_crosshair_enabled_mirror == true` and crosshair visibility reverts to `hud_root.visible`. Evidence: `tests/unit/hud_core/test_crosshair_widget.gd`
...
**AC-HUD-9.1 [Logic] [BLOCKING]** **REV-2026-04-25 — Performance API corrected**: GIVEN the Restaurant reference scene running on Iris Xe Gen 12 hardware at 810p, WHEN HUD frame cost is sampled over 300 consecutive frames at idle (no signal emissions, only prompt-strip poll), THEN p95 frame cost ≤ 0.3 ms as measured by **HUD-scoped `Time.get_ticks_usec()` bracketing** in a test-only debug build (entry timestamp at start of `_process`, exit at end; difference recorded into a sample array; p95 computed in the test harness). `Performance.TIME_PROCESS` is NOT used — it reports the WHOLE frame's `_process` time, not HUD-only, and cannot isolate a HUD regression. **Hardware fallback**: if Iris Xe Gen 12 is unavailable, evidence may be collected on the closest available hardware profile with a documented scaling factor per ADR-0008 Slot 7 normalization table; CI runner baseline `hardware_profile: ci-runner` MUST be documented as the reference before sprint start. Evidence: `tests/unit/hud_core/test_performance_budget.gd` (headless timing harness with `OS.is_debug_build()` guard).

**AC-HUD-9.2 [Logic] [BLOCKING]**: GIVEN a synthetic worst-case frame (5 simultaneous signal emissions: `player_damaged`, `ammo_changed`, `weapon_switched`, `gadget_equipped`, `player_health_changed` with critical-state threshold crossing), WHEN F.5 is computed, THEN `C_frame ≤ 0.259 ms` as asserted by the formula: `C_poll + 5 × C_label_max + C_flash_max = 0.004 + 5 × 0.050 + 0.005 = 0.259 ms`. The test emits all 5 signals on a single frame and asserts measured frame time does not exceed 0.3 ms on the reference hardware profile. Evidence: `tests/unit/hud_core/test_performance_budget.gd`
...
**AC-HUD-9.4 [Logic] [BLOCKING]** **REV-2026-04-25 — implementable rule**: GIVEN HUD Core source files in `src/ui/hud_core/**/*.gd` (excluding `tests/`), WHEN a CI static-analysis script (`tools/ci/check_process_text_guards.gd`) parses the GDScript AST and inspects each `_process` function body, THEN zero `.text = ...` assignments are found that are NOT immediately preceded (within the same `if` block) by a string-equality change-guard (e.g., `if new_text != _label.text:` or `if new_text != _last_text:`). Multi-line grep was previously specified and is not implementable across function-body boundaries; AST-level analysis is the corrected approach. **Stricter alternative (acceptable substitute)**: if the AST tool is unavailable, the rule may be implemented as "no `.text =` anywhere in `_process` at all" — implementers route Label updates exclusively through signal handlers, never `_process`. Evidence: `tests/unit/hud_core/test_forbidden_patterns.gd` (AST gate or stricter "_process-free `.text =`" gate).
...
**AC-HUD-11.5 [UI] [ADVISORY]**: GIVEN the AccessKit integration is configured per ADR-0004 Gate 1 (deferred to Polish), WHEN HUD numeral Labels update during gameplay, THEN the AccessKit live-region property (`accessibility_live` — exact name to be confirmed per Coord item §C.5#4) is set to suppress screen-reader announcements for non-critical numeral changes. Deferred to Polish sprint per ADR-0004 §10. Evidence: `production/qa/evidence/hud_core/walkthrough_accesskit_numerals_<date>.md` + lead programmer sign-off.
...
**AC-HUD-12.2 [Integration] [BLOCKING]**: GIVEN a `LOAD_FROM_SAVE` trigger frees the HUD scene and re-instantiates it, WHEN `_ready()` completes on the new instance, THEN all widget mirror variables (`_current_health`, `_max_health`, `_weapon_id`, `_ammo_current`, `_ammo_reserve`, `_gadget_id`, `_was_critical`, `_takedown_eligible`, `_pending_flash`) are at their zero/empty defaults before any LSS restore signals arrive. Evidence: `tests/integration/hud_core/test_save_load_lifecycle.gd`
...
### OQ-HUD-7 [BLOCKING for VS — fantasy/playtest decision] **NEW REV-2026-04-25**

**TAKEDOWN_CUE rendered behaviour at MVP — text vs glyph vs HIDDEN**: the original CR-12 specified `TAKEDOWN_CUE` renders the literal string `tr("HUD_TAKEDOWN_AVAILABLE")` ("TAKEDOWN AVAILABLE") whenever a SAI takedown opportunity is eligible. Adversarial review (game-designer + creative-director synthesis, 2026-04-25) flagged this as contradicting the §Player Fantasy "cockpit-dial" framing — the HUD is supposed to **confirm** state Eve already feels (peripheral readout), not **invite** a tactical action the player has not yet decided to take. Three resolution paths exist: **(A) Demote to HIDDEN at MVP** (current REV-2026-04-25 default — `_takedown_eligible` latch is implementation-ready but resolver returns HIDDEN; SAI body-language cues alone signal opportunity; first-playtest measures whether players reliably read the rear-facing-guard-within-range affordance); **(B) Replace text with non-instructional glyph** (small Parchment glyph at prompt-strip position; period-appropriate; preserves eligibility cue without instructional voice — requires Art Director input on glyph design and an Art Bible §7 amendment); **(C) Keep "TAKEDOWN AVAILABLE" text as-is** (accept fantasy tension as a deliberate accessibility/playability exception, document in §Player Fantasy). **Default if unresolved**: Path A (HIDDEN at MVP per REV-2026-04-25). **Owner**: game-designer + creative-director + playtest QA. **Resolution path**: first 5 playtest sessions on the Restaurant reference scene with Path A; if takedown-opportunity-discovery rate drops below 60% (vs Path C baseline, which is industry-standard hit-marker discovery rate), promote Path B for the next playtest; if Path B fails, fall back to Path C and document the fantasy exception. **Scope**: latch + signal subscription stay in MVP regardless; only the visible rendering is the open question.
