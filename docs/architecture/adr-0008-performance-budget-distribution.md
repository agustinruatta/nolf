...
- **Pooled residual 0.8 ms across 6 not-yet-designed systems** is thin — roughly 0.13 ms per system average. Systems with heavier runtime needs (Civilian AI at crowd density, Mission Scripting during scripted sequences) may need to renegotiate by pulling from the 1.6 ms reserve via a future ADR-0008 amendment.
...
2026-04-23 (initial draft; specialist validation folded in; all four validation gates pending execution to promote Proposed → Accepted).
