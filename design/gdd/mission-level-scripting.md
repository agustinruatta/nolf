...
**CR-10 — WorldItem cache placement policy (MLS-authored, Level Designer executes).** Inventory-locked caps are total-mission counts (8 pistol + 2 dart-off-path + 3 medkit + 1 rifle-carrier-per-section + 1 Parfum-Restaurant). MLS §C.5 defines placement policy (off-path minimums, distribution ratios, mission-gadget pinning). Level Designer places `WorldItem` scene instances per policy. **MLS does NOT spawn WorldItems at runtime** — scene-authored static placements only. Mission scripts may `queue_free` a WorldItem via a scripted beat but may not spawn new ones.
...
**CR-17 — MLS WILL register as autoload at ADR-0007 slot #9 (amendment is BLOCKING pre-implementation gate; not yet drafted as of 2026-04-24).** Will register as `MissionScripting` of type `class_name MissionScriptingService extends Node` at load-order line 9 (after F&R at line 8 per F&R coord item #1; Combat at line 7). MLS will subscribe to F&R's `respawn_triggered` — load-order dependency requires F&R first. ADR-0007's CURRENT canonical table has 7 entries (Events through Combat); slots #8 (F&R) and #9 (MLS) are FORWARD CLAIMS pending bundled amendment per /design-review godot-specialist finding #12. **No MLS implementation story may enter In Progress until the bundled ADR-0007 amendment lands** (preferably as a single change with F&R's slot-#8 amendment per coord item #1). Per ADR-0007 Cross-Autoload Reference Safety rule 4: MLS MUST NOT reference other autoloads in `_init()` — all cross-autoload setup in `_ready()`.
...
| **Medkit pickups** | **3 total mission** | At most 1 per section; place at section midpoint relative to section flow (not at entrance, not at exit); not in combat-committed zones |
...
- **ADVISORY** (playtest-gated per Inventory OQ-INV-2): pistol ≤ 8, dart ≤ 2, medkit ≤ 3 mission totals.
...
| Pistol / Dart / Medkit total caps | Counts ≤ 8 / 2 / 3 mission totals | ADVISORY (playtest-gated) |
...
| Mission totals (hard cap, Inventory-locked) | `pistol_total`, `dart_total`, `medkit_total` | `int` | 8, 2, 3 | Absolute mission-wide totals (Inventory CR-10) |
...
| **ADR-0007 Autoload Load Order Registry** | Accepted (w/ amendment queue) | MLS registers as autoload `MissionScripting` at line 9 (after F&R at line 8 per F&R's own coord; Combat at line 7). Slot #8 originally reserved for Civilian AI / MLS / DC shared — F&R claimed slot #8 first, so MLS shifts to slot #9. Load-order dependency: MLS subscribes to F&R's `respawn_triggered`. | **Coord item**: ADR-0007 amendment naming MLS at line 9 (pre-implementation BLOCKING gate per CR-17; coordinate with F&R's own slot-#8 amendment as bundled change) |
...
1. **ADR-0007 amendment — MLS autoload at line 9** (coord with F&R's own slot-#8 amendment; ideally bundled as one change). Per CR-17.
...
| `medkit_per_section_max` | **1** | [1, 2] | 0: medkit anywhere except designated midpoints forbidden. 2: opens bunching — 2 medkits in a forgiving section, 0 in a punishing one — violates spec. |
...
| `medkit_total` | **3 mission** | Inventory Coord item #5 |
...
- **MLS-spawned content**: MLS does NOT spawn runtime content at MVP (CR-10) — WorldItems and guards are scene-authored. Each WorldItem and guard CharacterBody3D must write its stencil tier per ADR-0001:
  - Guards and civilians (including per-variant uniforms per CR-19): **Medium tier**.
...
- **AC-MLS-7.3** [Config/Data] [ADVISORY]: **GIVEN** 3 medkit placements, **WHEN** QA walks each section and logs positions, **THEN** no section has >1 medkit; each at section midpoint; none in combat-committed zones. Evidence: `production/qa/smoke-2026-04-24.md`.
...
- **AC-MLS-12.1** [Logic] [BLOCKING]: **GIVEN** ADR-0007 amended to name `MissionScripting` at slot #9 (after F&R at slot #8), **WHEN** game initializes and `MissionScripting._ready()` executes, **THEN** `Events` autoload (slot #1), F&R autoload (slot #8), and all preceding autoloads are already initialized; `MissionScripting` connects to `Events.section_entered` + `Events.respawn_triggered` without null-ref crash, confirming load-order correctness. Evidence: `tests/unit/mission/autoload_order_test.gd`.
...
| # | Item | Owner |
|---|------|-------|
| 1 | ADR-0007 amendment naming MLS at slot #9 (bundle with F&R's slot-#8 amendment) | technical-director |
