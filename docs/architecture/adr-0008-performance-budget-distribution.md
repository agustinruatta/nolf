...
Proposed *(amended 2026-04-28 per `/review-all-gdds` — Slot-8 panic-onset reserve allocation registered + autoload-cascade row count 7 → 10. Architectural decision unchanged.)*
...
| **Knowledge Risk** | MEDIUM — budget allocation itself is numerical (engine-agnostic), but the Iris Xe numbers below depend on Godot 4.6 defaults: Jolt as the 3D physics backend, D3D12 default on Windows, Forward+ Mobile renderer, Shader Baker first-run cost, and CompositorEffect scheduling. LLM training data predates 4.4–4.6. Every numeric claim below requires empirical verification against Godot 4.6 on the reference scene before Accepted (see Validation Criteria). |
...
| **Post-Cutoff APIs Used** | Jolt 3D physics (Godot 4.4 optional → 4.6 default); Shader Baker (Godot 4.5+, referenced for cold-boot accounting); CompositorEffect (Godot 4.6 outline pipeline, budget inherited from ADR-0001); D3D12 backend (Godot 4.6 Windows default, budget reserve sized for descriptor heap behaviour). |
| **Verification Required** | (a) Restaurant dense-interior reference scene must sustain 60 fps on Iris Xe Gen 12 @ 1080p × 0.75 scale with all 9 allocated slots active simultaneously (12 guards in COMBAT, outline + sepia active, music swap event, one save write). (b) Per-slot measurements must match the allocation below within tolerance specified per slot. (c) D3D12 post-stream warm-up allowance (3 frames) must be confirmed on Windows. |
...
- **Fixed Godot 4.6 defaults**: Jolt 3D physics (cannot substitute Godot Physics 3D without cost re-measurement), Forward+ Mobile renderer (Mobile profile required on Iris Xe — Forward+ Desktop is out of budget), D3D12 on Windows (descriptor heap reserve implications noted in Consequences).
...
- Hold a 10% reserve (1.6 ms) unallocated for OS jitter, Jolt first-contact spikes, D3D12 heap pressure, AudioServer reverb-swap CPU stalls, and unknowns.
...
| 9 | Reserve (unallocated, 10% margin) | — | **1.6** | OS jitter / shader compile / Jolt spikes / D3D12 heap pressure / AudioServer reverb-swap CPU stalls / unknowns |
...
│  │ cull     │ signals 1.0     │ sepia    │ contact   │no │    │   │dual  │D3D12,  │ │
│  │ trans-   │ GuardFire 0.5   │  0.5     │ margin    │n- │    │   │(CAI, │shader  │ │
│  │ parent   │                 │          │           │GF │    │   │Miss, │bake,   │ │
│  │          │ (Combat P95     │          │           │   │    │   │Doc,  │reverb  │ │
│  │          │  absorbed)      │          │           │   │    │   │Dial, │swap,   │ │
│  │          │                 │          │           │   │    │   │FR,SB)│...)    │ │
│  └──────────┴─────────────────┴──────────┴───────────┴───┴────┴───┴──────┴────────┘ │
│                                                                                      │
│  Constraint: directional shadow cascade = 1  (busts slot #1 if raised)              │
│  Constraint: D3D12 post-stream warm-up = 3 frames over-cap allowed                  │
└──────────────────────────────────────────────────────────────────────────────────────┘
...
- **Pooled residual 0.8 ms across 6 not-yet-designed systems** is thin — roughly 0.13 ms per system average. Systems with heavier runtime needs (Civilian AI at crowd density, Mission Scripting during scripted sequences) may need to renegotiate by pulling from the 1.6 ms reserve via a future ADR-0008 amendment. **Sub-claims registered as of 2026-04-28** (informative — not contractual): CAI 0.30 ms p95 (`civilian-ai.md:394`); MLS 0.1 ms steady-state + 0.3 ms peak (`mission-level-scripting.md:528`); DC 0.05 ms peak event-frame (`document-collection.md:117`); F&R ~0 ms outside flow (`failure-respawn.md:272`); Dialogue & Subtitles + Signal-Bus dispatch overhead unspecified. Steady-state sum ≈ 0.45 ms (within 0.8 ms cap); panic-onset frame is the worst case and is governed by the reserve carve-out registered in §Risks (2026-04-28 amendment).
- **Windows/D3D12 post-stream warm-up allowance adds complexity** to CI gate enforcement — the gate must track section-transition timing and skip the first 3 frames. Engineering cost borne by devops-engineer (perf-gate job).
...
| **PostProcessStack cold-boot exceeds autoload budget** (compositor pipeline registration + D3D12 descriptor heap setup are the dominant contributors) | MEDIUM | LOW | 50 ms autoload budget absorbs the specialist-estimated 5–15 ms Vulkan + 5–10 ms additional D3D12 cost with margin. If the number is ever profiled over cap, ADR-0007 and ADR-0001 are revisited together. Non-blocking — first-run cost is off the 60 fps clock. |
| **D3D12 descriptor heap stall mid-session** (new shader variants or texture bindings crossing heap page boundary after a section transition) | MEDIUM | MEDIUM | 1.6 ms reserve absorbs single-frame stalls on Vulkan. On D3D12, the 3-frame post-stream warm-up window allows up to ~50 ms over-budget immediately after a transition. Shader Baker pre-compilation (ADR-0005 Gate 5) is the primary mitigation — bakes all variants at build time, eliminating mid-session variant compilation. |
...
- **Evidence**: `production/qa/evidence/adr-0008-gate-1-iris-xe-[YYYY-MM-DD].md` with: CPU model, GPU model, OS + graphics driver version, physics backend (Jolt), graphics API (Vulkan/D3D12), per-slot histogram with p50/p95/p99/max, total frame-time histogram, reference scene commit hash.
...
- **Scope**: On Windows/D3D12, run the reference scene with a scripted section transition mid-capture (Plaza → Restaurant). Measure frame time for 10 frames immediately post-transition.
- **Pass criterion**: Frames 1–3 post-transition may exceed 16.6 ms (warm-up allowance); frames 4–10 must return under 16.6 ms. If frames 4+ stay over, D3D12 descriptor heap pressure is a ship-blocking issue and Shader Baker configuration (ADR-0005 Gate 5) must re-run.
- **Evidence**: `production/qa/evidence/adr-0008-gate-3-d3d12-warmup-[YYYY-MM-DD].md` with per-frame timings frames 0–15 around the transition event.

### Gate 4 — Autoload boot cold-start (both platforms)

- **Scope**: Cold-launch the game on Iris Xe Gen 12 (Linux Vulkan) and on representative Windows D3D12 hardware. Measure wall-clock time from process start to `_ready()` completion on the last autoload (PostProcessStack).
...
- **ADR-0005** (`docs/architecture/adr-0005-fps-hands-outline-rendering.md`) — FPS hands SubViewport rendering absorbed into Slot #1 (Rendering) alongside main-viewport opaque pass; Gate 5 (Shader Baker × material_overlay) is the primary mitigation for D3D12 post-stream warm-up risk.
...
2026-04-28 (`/review-all-gdds` amendment — Slot-8 panic-onset reserve carve-out + autoload-cascade row 7 → 10. Architectural decision unchanged; budget envelope unchanged; reserve allocation policy made explicit. Prior: 2026-04-23 (initial draft; specialist validation folded in; all four validation gates pending execution to promote Proposed → Accepted)).
