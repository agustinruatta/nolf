...
**CR-6 — The player cannot save during a non-GAMEPLAY context.** When `InputContext.current()` is one of `CUTSCENE`, `DOCUMENT_OVERLAY`, `MODAL`, or `LOADING`, the Pause Menu's "Save Game" and Quicksave (F5) inputs are not routed. Mission Scripting notifies SaveLoad when the cutscene ends; Document Overlay UI's `close_document()` lifecycle (its `IDLE` state) precedes any save; modal scaffolds (Quit-Confirm, Save-overwrite, photosensitivity warning) and section-loading are mid-transition states with no stable scene tree to capture. **No deferred save is enqueued in any case** — the keypress is silently dropped per Input GDD context-gating rules; the player must close the overlay/modal/cutscene and return to GAMEPLAY before saving. *(2026-04-28: extended beyond CUTSCENE per `/review-all-gdds` 2026-04-28 finding W6 — F5 precondition `Overlay.state == IDLE` was not explicit. Document Overlay UI's Lectern Pause is incompatible with mid-read save state per Pillar 1 / Document Overlay UI CR-7; LOADING is mid-section-tear-down with the previous scene's owning systems being freed; MODAL is a transitional UI state that forbids gameplay-state mutation per ADR-0004 Implementation Guideline. Pause Menu's `MENU` and `PAUSE` contexts ARE save-eligible — they are the canonical save UI contexts.)*
...
|---|---|---|---|
| **Mission & Level Scripting** | Triggers save on `section_entered`; triggers load after slot selection | `MissionState`: `section_id`, `objectives_completed: Array[StringName]`, `triggers_fired: Dictionary[StringName, bool]` (per MLS CR-6 + OQ-MLS-12 type-lock), **`fired_beats: Dictionary[StringName, bool]`** (added per ADR-0003 amendment A4 2026-04-27 — closes B8 from /review-all-gdds 2026-04-27; enforces MLS CR-7 savepoint-persistent-beats invariant) | Restores mission progress, marks suppressed triggers as "already fired", restores fired beats so they do NOT re-trigger on section reload, loads correct section via Level Streaming |
| **Failure & Respawn** | Triggers save (slot 0) on `player_died`, before respawn sequence; triggers load from slot 0 during respawn | Reads from all systems; assembles full `SaveGame`. **Owns its own sub-resource `FailureRespawnState`** on `SaveGame` (added per ADR-0003 amendment A4 2026-04-27 — closes W6 from /review-all-gdds 2026-04-27; closes F&R coord item #1). F&R is sole writer of this field per F&R CR-3; MLS does NOT touch it during the per-owning-system `capture()` chain (CR-15). | Calls `SaveLoad.load_from_slot(0)`, hands deep-duplicated state to each system (including `FailureRespawnState` to F&R itself), initiates scene reload |
...
- **F5 (Quicksave)**: fire immediately if `InputContext.current() in {GAMEPLAY, MENU, PAUSE}` (extended 2026-04-28 per CR-6 — formerly only excluded CUTSCENE; now also excludes `DOCUMENT_OVERLAY`, `MODAL`, `LOADING`). No confirmation dialog (expediency is the point). On success: brief HUD notification ("Quicksave to slot 0"). On failure: `Events.save_failed` → Pause Menu dialog. If gated: silent no-op (the keypress is dropped per Input GDD context-gating rules — there is no error dialog, no toast, no "wait for current state to finish"; the player must close the overlay/modal/cutscene to enable F5 again).
...
- **If the player triggers Quicksave (F5) during a non-GAMEPLAY/MENU/PAUSE context** (`CUTSCENE`, `DOCUMENT_OVERLAY`, `MODAL`, `LOADING`) → input is not routed per CR-6 (extended 2026-04-28 per `/review-all-gdds` finding W6). **Resolution**: Quicksave is silent no-op. No error dialog; the keypress is simply ignored per Input GDD context-gating rules. The player must close the overlay/modal/cutscene/wait for LOADING to complete before F5 will register. **Specifically for `DOCUMENT_OVERLAY`**: Document Overlay UI's `IDLE` state is the only save-eligible overlay state; OPENING / READING / CLOSING all gate F5. The same rule applies to manual save via Pause Menu — Pause Menu cannot be opened during DOCUMENT_OVERLAY (the overlay swallows Esc per Document Overlay UI CR-7's `_unhandled_input` + `ui_cancel` discipline), so the gating is naturally enforced.
- **If the player triggers Quicksave (F5) during Document Overlay UI's READING state** → identical to above — Document Overlay UI has pushed `InputContext.DOCUMENT_OVERLAY`; F5 is silently dropped. **Resolution**: intended. The Lectern Pause is a complete interruption of gameplay; saves during overlay would capture mid-read state with the document modal scene tree active, which is incompatible with the post-load resume contract (CR-8 fades to black and reloads the section — there is no "resume mid-overlay-read" affordance, by design per Document Overlay UI's Lectern Pause player fantasy).
- **If the player triggers Quicksave (F5) during a section transition** (`InputContext.current() == LOADING`, between LS step 1 and step 11) → input is not routed per CR-6. **Resolution**: silent no-op. The previous section's owning systems are being freed; capturing state during this window would produce a stale-reference `SaveGame`. The player can F5 again the moment `section_entered` fires and `InputContext` pops back to `GAMEPLAY`.
- **If the player triggers Quicksave (F5) during a modal dialog** (Quit-Confirm, Save-overwrite, photosensitivity boot warning, save-failed dialog, resolution-revert countdown) → input is not routed per CR-6. **Resolution**: silent no-op. Modal dialogs are transient UI states that block gameplay-state mutation; allowing F5 to fire during a Quit-Confirm modal would race the application-shutdown path. The player closes the modal first (Esc dismisses; Enter confirms), then F5 is enabled in the underlying context.
...
- **Scene fade-to-black on load** — owned by Level Streaming (system 9). Timing owned by Level Streaming GDD §Tuning Knobs (`fade_out_duration`, `fade_in_duration`, `load_hold_budget_ms`, `total_transition_budget_s`) — consult there for the canonical spec. (Was previously stated inline as "0.3 s fade out → section load → 0.5 s fade in"; superseded 2026-04-21 by the LS senior-director ruling that replaced the dissolve with a 2-frame hard-cut grammar; pointer-only wording adopted 2026-04-27 per `/consistency-check` C2 / LS-Gate-4 Option B closure.)
