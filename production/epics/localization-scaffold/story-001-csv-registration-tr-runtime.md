...
**Completed**: 2026-05-01
**Criteria**: 8/8 passing — covered by 12 test functions in `tests/unit/foundation/localization_runtime_test.gd`.
**Test results**: Localization runtime → 12/12 PASS. Full suite → 214/214 PASS (was 202 + 12 new).

### Files added (9 stub CSVs + 1 test)
- `translations/hud.csv` (3 keys: health, objective, compass)
- `translations/menu.csv` (5 keys: start_mission, continue, options, credits, quit)
- `translations/settings.csv` (4 keys: master_volume, fullscreen, mouse_sensitivity, subtitles)
- `translations/meta.csv` (3 keys: app title, copyright year, studio name — brand strings)
- `translations/dialogue.csv` (3 keys: guard alert, guard combat, eve interact)
- `translations/cutscenes.csv` (2 keys: opening title, mission_brief heading)
- `translations/mission.csv` (3 keys: collect_documents, complete title, failure title)
- `translations/credits.csv` (5 keys: section headers — design, programming, art, audio, thanks)
- `translations/doc.csv` (2 keys: example title + body — placeholder for Document Collection epic)
- `tests/unit/foundation/localization_runtime_test.gd` (12 functions covering all 8 ACs)

### Files modified
- `project.godot` — added `[internationalization]` block listing all 10 domain translation artifacts + `locale/fallback="en"`
- `translations/overlay.csv` — migrated 2-segment keys (`overlay.dismiss_hint`, `overlay.scroll_hint`) to 3-segment compliant keys (`overlay.footer.dismiss_hint`, `overlay.footer.scroll_hint`) to comply with TR-LOC-002 + AC-7 regex enforcement. No production code referenced the old keys (Document Overlay UI is future scope), so the migration is safe.

### Auto-generated by Godot import
- `translations/{hud,menu,settings,meta,dialogue,cutscenes,mission,credits,doc}.{en,# context}.translation` — Godot's CSV importer generates these per-locale and per-context artifacts on first editor load. Total: 18 new .translation files + 18 .csv.import metadata files. These are the binary translation tables `TranslationServer` actually loads at runtime.

### Deviations
- **ADVISORY — overlay.csv 2-segment key migration**: Sprint 01 authored `overlay.dismiss_hint` and `overlay.scroll_hint` before LOC-001's 3-segment rule existed. Migrated inline (no production-code references existed). Documented for transparency; future stories should not need to migrate.
- **ADVISORY — ADR-0004 Proposed status**: ADR-0004 is overall Proposed pending Gate 5 (BBCode → AccessKit serialization, runtime AT testing). Localization-relevant scope (G1-G4) is CLOSED. The epic explicitly authorizes localization stories to proceed; story header documents this. AC-7 (3-segment regex) is one of the locked rules from G4 (auto_translate_mode enum identifiers).

### Tech debt logged
- None. Story scope fully delivered.

### Critical proof points verified
- All 10 domain CSVs registered + importable + present at runtime
- `tr("menu.main.start_mission")` → "Start Mission" (known-key resolution)
- `tr("nonexistent.fake.key")` → key verbatim (loud-and-obvious miss)
- Setting locale to "fr" (not loaded) → tr() returns en value (fallback config works)
- Every CSV row has non-empty `# context` (Rule 3)
- Every key matches `^[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*(\.[a-z0-9_]+)*$` (3-segment minimum)
- Pre-existing overlay.csv keys still resolve (regression guard)

### Unblocks
- **LOC-002** (pseudolocalization CSV + dev workflow + export filter)
- **LOC-003** (plural form support)
- **LOC-004** (auto_translate_mode + NOTIFICATION_TRANSLATION_CHANGED discipline)
- **LOC-005** (anti-pattern fences + lint guards)
- All UI / HUD / Menu / Document / Mission / Cutscenes content stories — they have a CSV to write keys into
