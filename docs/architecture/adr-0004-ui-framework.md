...
- **Gate 2** ⚠️ **REOPENED-AND-RECLOSED 2026-05-03** — Sprint 06 HC-001 verification proved `Theme.fallback_theme` does NOT exist on `Theme` in Godot 4.6 (the 2026-04-27 godot-specialist sign-off was incorrect — the property does not exist on the Theme class at all). Cross-theme inheritance in Godot 4.x is achieved via the `Control` parent chain — a child Control inherits its parent Control's theme unless it sets its own. Resolution: HC-001 was relaxed to align with engine reality; per-surface themes are assigned to surface root Controls and descendants inherit through the node tree, not via a `Theme` property. Implementation Guideline 6, Decision §1, the architecture diagram's `fallback_theme` arrow, and the verification table item (2) wording are now misleading and should be amended to "surface root Control assigns surface theme; descendants inherit via Control parent chain (Godot's standard mechanism)." Verified 2026-05-03 against shipped HUD scene tree (`tests/unit/feature/hud_core/hud_core_scaffold_test.gd` Theme load + Control discipline tests pass).
...
- **Gate 4** ✅ CLOSED 2026-04-27 — `Node.AUTO_TRANSLATE_MODE_*` enum identifiers verified. Re-verified 2026-05-03 LOC-004 (8/8 tests including enum value sanity check confirming `AUTO_TRANSLATE_MODE_ALWAYS == 1`). Plural CSV format separately verified 2026-05-03 LOC-003: Godot 4.6 uses `?plural` marker column + `?pluralrule` directive row + row-repetition (NOT `en_0`/`en_1`/`en_other` locale-suffixed columns). Reference: `translations/hud.csv`; tests: `tests/unit/foundation/localization_plural_forms_test.gd` (8/8 PASS).
