...
#
# SIGN CONVENTION (Godot 4 Camera3D, default forward = -Z, right-hand rule)
#   +rotation.x rotates the forward vector toward +Y → camera looks UP.
#   Mouse down (positive relative.y) → _unhandled_input SUBTRACTS from
#     rotation.x → camera looks down → clamped at -85° (deg_to_rad(-85.0)).
#   Mouse up (negative relative.y) → rotation.x increases → camera looks up
...
## camera.rotation.x to -85° (looking down). Mouse-down subtracts from
## rotation.x; the lower bound is the clamp at the look-down extreme.
## Implements: PC-002 AC-7.2.
func test_player_camera_pitch_clamps_at_negative_85_degrees_when_looking_down() -> void:
	# Arrange
...
## camera.rotation.x to +85° (looking up). Mouse-up adds to rotation.x; the
## upper bound is the clamp at the look-up extreme.
## Implements: PC-002 AC-7.2.
func test_player_camera_pitch_clamps_at_positive_85_degrees_when_looking_up() -> void:
	# Arrange
	var inst: PlayerCharacter = _instantiate_player()
	var cam: Camera3D = inst.get_node(^"Camera3D") as Camera3D
	assert_object(cam).override_failure_message(
