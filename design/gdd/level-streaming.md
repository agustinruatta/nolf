...
### Per-Section Subscriber Lifecycle Guarantees (NEW 2026-04-28 night — D&S Phase 2 propagation per dialogue-subtitles.md §F.6 P4 / EC-DS-2 + EC-DS-3)

Per-section nodes (i.e., systems that are NOT autoload — Dialogue & Subtitles per ADR-0007 closed slot list, HUD State Signaling, Document Overlay UI, Document Collection, etc.) instantiate at section load and tear down at section unload. LS guarantees the following lifecycle invariants for any per-section subscriber that publishes a "started/finished" or "open/close" pair on the Signal Bus:

- **EC-DS-2 — `_exit_tree` runs before scene swap.** When `transition_to_section(_, _, _)` is invoked (any reason), at step 4 of the 12-step sequence (BEFORE `current_section.queue_free()`), LS recursively walks `current_section`'s scene tree and ensures every node's `_exit_tree()` callback fires. For a node that has emitted a "started" signal but not yet a "finished" signal (e.g., D&S has emitted `dialogue_line_started` but not yet `dialogue_line_finished` because the player triggered F9 quick-load mid-bark), the per-section node's `_exit_tree` MUST emit the paired "finished" signal **before** `queue_free()` is called. This prevents orphaned ducking state on Audio (Audio's duck timer would otherwise wait forever for `dialogue_line_finished`, leaking ducking into the next section). LS does not enforce this contract by introspection — each per-section subscriber owns its own `_exit_tree` discipline; the LSS guarantee is only that `_exit_tree` IS reached before `queue_free`. Closes D&S `dialogue-subtitles.md` EC-DS-2 / Cluster F F.4 lifecycle guard. Pattern reference for authors: D&S §C.16 `_exit_tree` snippet.

- **EC-DS-3 — Audio autoload init order before per-section subscribers.** Per-section nodes that subscribe to Audio domain signals (e.g., D&S subscribes to `Events.dialogue_line_started` / `_finished` for ducking, indirectly via Audio's subscriber set) MUST be guaranteed that AudioManager has completed its own `_ready()` initialization (autoload boot, registered in ADR-0007 §Canonical Registration Table) before the per-section subscriber's `_ready()` runs. ADR-0007 IG3 already enforces autoload-load-order strictness — autoloads complete `_ready()` calls in slot order BEFORE the main scene's `_ready()` fires. By extension, when LS invokes step-7 (`current_section = new_section_instance`) and step-8 (`add_child(new_section_instance)`), every autoload (including Audio) has been initialized for the duration of the application — there is no race window. Per-section nodes MAY assume `AudioManager` is non-null and bus state is initialized at any point during their `_ready()`. Closes D&S `dialogue-subtitles.md` EC-DS-3 / Cluster G G.1 boot-order race.

- **EC-DS-singleton — Single-instance lifetime per per-section system.** Per-section systems that hold scene-tree-rooted state (e.g., D&S's `DialogueAndSubtitles` node, Document Collection's `DocumentCollection` node) MUST exist as **at most one instance** within `current_section` at any time. Because LS frees `current_section` at step 4 BEFORE adding `new_section_instance` at step 8, there is no window during which two per-section nodes of the same class coexist. Per-section authoring lints (Tools-Programmer scope, joins MLS section-validation CI per `mission-level-scripting.md` §C.5) MUST verify that each `*.tscn` section root contains AT MOST one instance of each NOT-autoload class. CI lint rule: `grep -c '^class_name DialogueAndSubtitles' [section_root_node].tscn` returns ≤ 1. Closes D&S `dialogue-subtitles.md` §F.6 P4(c) single-instance guarantee.

These three guarantees compose: per-section subscribers can subscribe to autoload signals in `_ready()` knowing the autoload is initialized (EC-DS-3), can emit pending "finished" signals in `_exit_tree()` knowing they will run before `queue_free` (EC-DS-2), and can hold per-instance state knowing no parallel instance exists during the swap window (EC-DS-singleton). Other downstream systems with similar pair-emit patterns (e.g., HSS state lifecycle, Document Overlay UI open/close) MAY rely on the same guarantees.
